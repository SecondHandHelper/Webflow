...
async function fillForm(itemId) {
  try {
    ...
    if (data.imagesv2) {
      ...
      showImageV2AndHideSiblings(data.imagesv2.find(img => img.name === 'extraImage.0'), 'brandTagImage');
      showImageV2AndHideSiblings(data.imagesv2.find(img => img.name === 'extraImage.1'), 'materialTagImage');
      // loop through extraImage.x and defectImage.y and show them by closning the html element
      showExtraImagesV2(data.imagesv2.filter(img => img.name.match(/extraImage\.[23456789]/)));
