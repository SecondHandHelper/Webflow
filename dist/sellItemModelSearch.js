...
var u=d(n);// CommonJS
"object"==typeof exports&&"undefined"!=typeof module?module.exports=u:"function"==typeof define&&define.amd?define(function(){return u}):o&&(this[o]=u)}}({kdF4Q:[function(e,t,n){let a,o;var r=e("@parcel/transformer-js/src/esmodule-helpers.js");r.defineInteropFlag(n),r.export(n,"setFieldValue",()=>s),r.export(n,"showSelectedModel",()=>d),r.export(n,"setupModelSearchEventListeners",()=>B),r.export(n,"displayFindModelDiv",()=>v);var i=e("./sellItem"),l=e("./sellItemHelpers");let s=(e,t)=>{document.getElementById(e).value=t||"",document.getElementById(e).dispatchEvent(new Event("input"))},d=e=>{(// Show selected model in search box
document.getElementById("findModelDescription").style.display="none",document.getElementById("findNewModel").style.display="flex",document.getElementById("findModelBoxEmpty").style.display="none",document.getElementById("findModelBoxFilled").style.display="flex",document.getElementById("findModelBoxFilled").setAttribute("data-model",JSON.stringify(e)),document.getElementById("findModelBoxImage").style.backgroundImage=`url('${e.coverImageSmall}')`,document.getElementById("findModelBoxNameCategory").innerText=`${e.brand}, ${e.category}`,document.getElementById("findModelBoxName").innerText=`${e.maiName}`,e.color?(document.getElementById("findModelBoxColor").style.display="block",document.getElementById("findModelBoxColor").innerText=`${(0,l.colorName)((0,l.capitalizeFirstLetter)(e.color))}`):document.getElementById("findModelBoxColor").style.display="none",e.gender)?(document.getElementById("findModelBoxGender").style.display="block",document.getElementById("findModelBoxGender").innerText=`${({Woman:"Dam",Man:"Herr"})[e.gender]||e.gender}`):document.getElementById("findModelBoxGender").style.display="none"},c=e=>t=>{S(),d(e),// Fill form with attributes from selected model
    document.getElementById(e.gender)?.parentElement?.click(),s("itemSize",t.target.innerText),s("itemMaterial",e.material),s("itemModel",e.maiName),s("itemOriginalPrice",e.originalPriceSek),e.collectionYear+1>=new Date().getFullYear()&&(document.getElementById("itemAge").selectedIndex=1,document.getElementById("itemAge").style.color="rgb(51, 51, 51)",document.getElementById("itemAge").dispatchEvent(new Event("input"))),e.category&&(document.getElementById("itemCategory").value=e.category,$("#itemCategory").trigger("change")),document.getElementById("itemColor").querySelector('[value="'+(0,l.capitalizeFirstLetter)(e.color)+'"]')&&(document.getElementById("itemColor").value=e.color,$("#itemColor").trigger("change"))},u=()=>{if(!a){let e=sessionStorage.getItem("models");a=JSON.parse(e)}return a},m=e=>u().filter(t=>t.maiName===e.maiName&&t.color===e.color&&t.category===e.category),g=e=>{window.scrollTo({top:0});let t=document.getElementById("modelSizeTemplate"),n=document.getElementById("modelSizeList"),a=document.getElementById("modelSizeList2");for(;n.firstChild;)n.removeChild(n.lastChild);for(;a.firstChild;)a.removeChild(a.lastChild);let o=JSON.parse(e.getAttribute("data-model"));if(1===o.sizes.length)return c(o)({target:{innerText:o.sizes[0]}});let r=m(o),i=r[0].sizes.slice()?.sort(),l=r[1]?.sizes.slice()?.sort(),s=l&&(l?.length!==i.length||!i.every((e,t)=>e===l[t]));for(let e of(s?(document.getElementById("modelSizeListHeading").style.display="block",document.getElementById("modelSizeList2Heading").style.display="block"):(document.getElementById("modelSizeListHeading").style.display="none",document.getElementById("modelSizeList2Heading").style.display="none"),s?r:r.slice(0,1)))for(let[o,r]of e.sizes.sort(h).entries()){let i=t.cloneNode(!0);for(let n of(i.id=`${t.id}_${o}`,i.addEventListener("click",linkClickTracker),i.style.display="block",i.addEventListener("click",function t(n){c(e)(n),this.removeEventListener("click",t)}),Array.from(i.getElementsByTagName("*"))))n.id=`${n.id}_${e.gender}_${o}`;("Woman"===e.gender?n:a).appendChild(i),document.getElementById(`modelSize_${e.gender}_${o}`).innerText=r}},f=e=>{document.getElementById("modelList").style.display="none",document.getElementById("modelSizeSelect").style.display="block",document.getElementById("modelSelectTitle").innerText="V\xe4lj storlek",g(e.currentTarget)},y=e=>{window.scrollTo({top:0});let t=document.getElementById("modelCardTemplate"),n=document.getElementById("modelResultList");for(;n.firstChild;)n.removeChild(n.lastChild);let a=e.filter(t=>{let n=e.find(e=>e.maiName===t.maiName&&e.color===t.color&&e.category===t.category&&e.gender!==t.gender);return n&&(t.multiGender="Dam / Herr"),!n||"Woman"===t.gender});for(let[e,o]of a.entries()){let a=t.cloneNode(!0);for(let n of(a.id=`${t.id}_${e}`,a.addEventListener("click",linkClickTracker),a.style.display="flex",a.style.cursor="pointer",a.setAttribute("data-model",JSON.stringify(o)),a.addEventListener("click",f),Array.from(a.getElementsByTagName("*"))))n.id=`${n.id}_${e}`;if(n.appendChild(a),document.getElementById(`modelImage_${e}`).src=o.coverImageSmall,document.getElementById(`brandNameCategory_${e}`).innerText=`${[o.brand,o.category].filter(e=>e).join(", ")}`,document.getElementById(`modelName_${e}`).innerText=`${o.maiName}`,o.color?document.getElementById(`modelColor_${e}`).innerText=`${(0,l.colorName)((0,l.capitalizeFirstLetter)(o.color))}`:document.getElementById(`modelColor_${e}`).style.display="none",o.multiGender||o.gender){let t={Woman:"Dam",Man:"Herr"};document.getElementById(`modelGender_${e}`).innerText=`${o.multiGender||t[o.gender]||o.gender}`}else document.getElementById(`modelGender_${e}`).style.display="none"}};function p(e,t){let n=e.maiName.toLowerCase(),a=t.maiName.toLowerCase();return n>a?1:n<a?-1:0}function h(e,t){let n=["XXS","XS","S","M","L","XL","XXL"];return n.indexOf(e)>-1&&n.indexOf(t)>-1?n.indexOf(e)-n.indexOf(t):e-t}function I(){o=window.scrollY,document.getElementById("header").style.display="none",document.getElementById("addItemFormDiv").style.display="none",document.getElementById("modelSelectError").style.display="none",document.getElementById("modelSizeSelect").style.display="none",document.getElementById("modelSelectTitle").innerText="V\xe4lj modell",document.getElementById("modelSelectDiv").style.display="block",document.getElementById("modelList").style.display="block",document.getElementById("modelSearchInput").value="",window.scrollTo({top:0});let e=sessionStorage.getItem("models");if(e)document.getElementById("modelSpinner").style.display="none",y(JSON.parse(e).sort(p));else{document.getElementById("modelSpinner").style.display="flex";let t=setInterval(()=>{(e=sessionStorage.getItem("models"))&&(clearInterval(t),document.getElementById("modelSpinner").style.display="none",y(JSON.parse(e).sort(p)))},1e3)}}let S=()=>{document.getElementById("addItemFormDiv").style.display="block",document.getElementById("modelSelectDiv").style.display="none",document.getElementById("modelSizeSelect").style.display="none",document.getElementById("header").style.display="block",window.scrollTo({top:o})},B=()=>{document.getElementById("findModelBoxEmpty").addEventListener("click",I),document.getElementById("findNewModel").addEventListener("click",I),document.getElementById("modelSelectClose").addEventListener("click",()=>{S()}),document.getElementById("modelSearchInput").addEventListener("input",()=>{let e=document.getElementById("modelSearchInput").value,t=JSON.parse(sessionStorage.getItem("models"));if(e&&e.length>0){let n=new Fuse(t,{includeScore:!0,keys:["maiName","category","color","maiColor","articleNumber","name"]}),a=n.search(e.replace(", "," "));y(a.map(e=>e.item))}else y(t.sort(p))}),document.getElementById("removeModelIcon").addEventListener("click",e=>{document.getElementById("findModelBoxEmpty").style.display="flex",document.getElementById("findModelBoxFilled").style.display="none",document.getElementById("findModelDescription").style.display="block",document.getElementById("findNewModel").style.display="none",e.stopPropagation(),e.preventDefault(),(0,i.rememberUnsavedChanges)()})},v=async e=>{if(featureIsEnabled("modelDB")){if("Eytys"===e){findModelDiv.style.display="block";let e=sessionStorage.getItem("models")?JSON.parse(sessionStorage.getItem("models")):void 0;if(!e){let t=await fetch("https://getbrandmodels-heypmjzjfq-ew.a.run.app?brand=Eytys",{method:"GET",headers:{"Content-Type":"application/json"}});e=await t.json(),sessionStorage.setItem("models",JSON.stringify(e))}}else findModelDiv.style.display="none"}}},{"./sellItem":"42EGR","./sellItemHelpers":"2G59s","@parcel/transformer-js/src/esmodule-helpers.js":"bNgzC"}],"42EGR":[function(e,t,n){let a;var o=e("@parcel/transformer-js/src/esmodule-helpers.js");o.defineInteropFlag(n),o.export(n,"rememberUnsavedChanges",()=>F),o.export(n,"isNoBgImage",()=>ea);var r=e("./sellItemHelpers"),i=e("qrcode"),l=o.interopDefault(i),s=e("./general"),d=e("./autocomplete-brands"),c=e("./sellItemModelSearch");let u=!1,m=getParamsObject();async function g(){// Grab values from form
...
if(document.referrer.includes("/sign-in")){await N();let e=sessionStorage.getItem("shippingMethod");e&&await callBackendApi("/api/users",{data:{data:{preferences:{shippingMethod:e}}},method:"PUT"});let t=user.current?.phoneNumber?.length;return location.href=t?"/item-confirmation":"/user-contact"}sessionStorage.removeItem("itemToBeCreatedAfterSignIn")}}async function S(){let e,t,n,o,i,f;let p=document.getElementById("qrCanvas");if(p&&(0,l.default).toCanvas(p,window.location.href,function(e){e&&console.error(e),console.log("success!")}),localStorage.removeItem("latestItemCreated"),sessionStorage.removeItem("itemValuation"),sessionStorage.getItem("itemToBeCreatedAfterSignIn")&&document.referrer.includes("/sign-in")){console.log(`showing spinner and waiting for item to be created when user is set ${user.current?.email}`),// ... if we are redirected here from the sign-in page and have a saved item that should be created
...
document.getElementById("wf-form-Add-Item").querySelectorAll("select").forEach(e=>{e.addEventListener("change",F)}),document.getElementById("wf-form-Add-Item").querySelectorAll("textarea").forEach(e=>{e.addEventListener("input",F)}),window.addEventListener("pageshow",e=>{e.persisted&&localStorage.getItem("newItem")&&setTimeout(async()=>{await H(null,JSON.parse(localStorage.getItem("newItem")),!0)},10)})}async function B(){let e="draft"===m.type?m.id:sessionStorage.getItem("newItemId")||await (0,r.requestUniqueId)();try{document.getElementById("addItemFormDiv").style.display="none",document.getElementById("loadingDiv").style.display="flex",document.getElementById("clearItemForm").style.display="none",document.getElementById("saveItemDraftDiv").style.display="none";let t=await A(e),n=await k(e,t);location.href=n}catch(e){errorHandler.report(e),console.error("addItem failed",e)}}function v({humanCheckNeeded:e,newMinMaxLog:t,lowValueSegment:n,lowValueCategory:a}){return e||t.match(/accept price is above max/i)&&!n&&!a}async function x(e,t){if(sessionStorage.getItem("itemToBeCreatedAfterSignIn")){let t=JSON.parse(sessionStorage.getItem("itemToBeCreatedAfterSignIn"));sessionStorage.setItem("itemToBeCreatedAfterSignIn",JSON.stringify({id:t.id,item:{...t.item,...e}}))}else{await callBackendApi(`/api/valuation/${t}`,{data:e});let n=JSON.parse(localStorage.getItem("latestItemCreated"));localStorage.setItem("latestItemCreated",JSON.stringify({...n,...e}))}}async function E(e,t){let{minPrice:n,maxPrice:a,decline:o,humanCheckNeeded:r,humanCheckExplanation:i,willNotSell:l,soldPrice:s,version:d,newMinPriceEstimate:c,newMaxPriceEstimate:u,newMinMaxLog:m,adjustmentAllowed:g,newBrand:f,newBrandCategory:y,valuatedBrandItems:p,brandMeanMax:h,brandAccuracy:I,brandCategoryAccuracy:S,fewBrand:B,brandMeanSold:E,brandCategoryMeanSold:b,highPriceVarBrandCategory:k,brandShareSold:C}=t||{};if(!n&&!o)return;let w={mlValuation:{decline:o,humanCheckNeeded:r,minPriceEstimate:n,maxPriceEstimate:a,humanCheckExplanation:i,willNotSellPrediction:l,soldPriceEstimate:s,modelVersion:d?.toString(),newMinPriceEstimate:c,newMaxPriceEstimate:u,newMinMaxLog:m,adjustmentAllowed:g,newBrand:f,newBrandCategory:y,valuatedBrandItems:p,brandMeanMax:h,brandAccuracy:I,brandCategoryAccuracy:S,fewBrand:B,brandMeanSold:E,brandCategoryMeanSold:b,highPriceVarBrandCategory:k,brandShareSold:C},newMinPriceEstimate:c||n,newMaxPriceEstimate:u||a,...o||v(t)?{}:{valuationStatus:"Completed",valuationDate:new Date().toISOString(),infoRequests:{price:{status:"Active",response:"",description:"Vi b\xf6rjar med startpriset, och justerar successivt ner till l\xe4gsta priset under s\xe4ljperioden p\xe5 30 dagar. V\xe4rderingen utg\xe5r fr\xe5n vad liknande s\xe5lts f\xf6r.",minPrice:c||n,maxPrice:u||a}}}};await x(w,e)}async function b(e,t,n){let a="Sold"===e.status?e.maxPriceEstimate:Math.min(e.maxPriceEstimate,Math.max(e.minPriceEstimate+150,10*Math.round((1.3*e.minPriceEstimate||0)/10))),o={valuationStatus:"Completed",valuationDate:new Date().toISOString(),newMinPriceEstimate:e.minPriceEstimate,newMaxPriceEstimate:a,infoRequests:{price:{status:"Active",response:"",description:"Vi b\xf6rjar med startpriset, och justerar successivt ner till l\xe4gsta priset under s\xe4ljperioden p\xe5 30 dagar. V\xe4rderingen utg\xe5r fr\xe5n vad liknande s\xe5lts f\xf6r.",minPrice:e.minPriceEstimate,maxPrice:a,type:"Valuation",source:"createdFromItem",adjustmentAllowed:!0}}};await x(o,n)}async function k(e,t){if(!e&&!t)return console.error("No item and no itemId, unexpected!!"),"/item-confirmation";if(m.id&&"draft"!==m.type){let n=await callBackendApi(`/api/items/${m.id}`),a=n.data;return await b(a,t,e),"/item-valuation"}try{let n=await callBackendApi("/api/valuation",{data:{itemId:e,item:t},requiresAuth:!1}),{minPrice:a,maxPrice:o,decline:r}=n.data||{};return await E(e,n.data),C(a&&o,r,v(n.data))}catch(e){console.error("Failed to get ml valuation",e)}return C()}function C(e,t,n){if(!e||n){if(sessionStorage.getItem("itemToBeCreatedAfterSignIn"))return"/sign-in";let e=user.current?.phoneNumber?.length;return e?"/item-confirmation":"/user-contact"}return"/item-valuation"}function w(){return{acceptPrice:null,age:null,brand:null,category:null,color:null,condition:null,defectDescription:null,defects:[],images:{},material:null,model:null,originalPrice:null,userValuationApproval:!0,sex:"Woman",size:null,userComment:null}}function M(){let e="",t=new Date,n=itemSize.value,a=itemMaterial.value?itemMaterial.value.trim():"",o=itemBrand.value?itemBrand.value.trim():"",r=itemModel.value?itemModel.value.trim():"",i=Number(itemOriginalPrice.value),l=itemAge.value,s=itemCondition.value,d=itemDefectDescription.value?itemDefectDescription.value.trim():"",c=itemUserComment.value?itemUserComment.value.trim():"",u=Number(itemLowestAcceptPrice.value),m=new Map().set("hole",hole.checked).set("stain",stain.checked).set("lostFit",lostFit.checked).set("pilling",pilling.checked).set("threadUp",threadUp.checked).set("colorChange",colorChange.checked).set("otherDefect",otherDefect.checked),g=[];"Anv\xe4nd, tecken p\xe5 slitage"===s&&m.forEach((e,t)=>{if(e){let e=f().get(t);g.push(e)}});for(var y=document.getElementsByName("Sex"),p=0;p<y.length;p++)y[p].checked&&(e=y[p].id);let h=document.getElementById("findModelBoxFilled"),I={};"flex"===h.style.display&&(I=JSON.parse(h.getAttribute("data-model")));let S=JSON.parse(localStorage.getItem("newItem")||"{}").images;return{user:authUser.current?.uid||null,createdAt:t.toISOString(),status,shippingStatus:"Not sent",sex:e,size:n,material:a,color:itemColor.value,category:itemCategory.value,brand:o,model:r,originalPrice:i,age:l,condition:s,defects:g,defectDescription:d,userComment:c,acceptPrice:u,preferences:{userValuationApproval:!0},modelVariantFields:I,images:S}}async function L(){// If first time: User chooses shipping method preference in sell item form
let e="Service point";return user.current?.preferences?.shippingMethod?e=user.current?.preferences?.shippingMethod:authUser.current?await callBackendApi("/api/users",{data:{preferences:{shippingMethod:e}},method:"PUT"}):sessionStorage.setItem("shippingMethod",e),e}async function A(e,t="New"){let{modelVariantFields:{coverImage:n,atVariantId:a},images:o,...r}=M(),i=await L();n&&(o.modelImage=n);let l=m.id&&"draft"!==m.type?{createdFromItem:m.id}:{},s=!!Object.keys(l).length,d={...r,status:t,..."Draft"===t?{draftSource:s?"Mai purchase":"Sell item"}:{},shippingMethod:i,images:o,...l,atVariantId:a,version:"2"};if(authUser.current){let t=await callBackendApi(`/api/items/${e}`,{data:{item:d}});//Archive if "createdFromItem" is same seller
if(await p(),await T(),localStorage.removeItem("newItem"),sessionStorage.removeItem("newItemId"),localStorage.setItem("latestItemCreated",JSON.stringify(t.data)),s){let e=await P(m.id);e===authUser.current.uid&&await db.collection("items").doc(m.id).update({archived:!0})}}else sessionStorage.setItem("itemToBeCreatedAfterSignIn",JSON.stringify({id:e,item:d}));return{...d,id:e}}async function P(e){console.log("getCreatedFromItemUserId()");let t=await db.collection("items").doc(e).get();return t.exists?t.data().user:null}async function T(){let e="2024-08-18">=new Intl.DateTimeFormat("se-SV").format(new Date);e&&"noCommission"===getCookie("noCommissionCampaignCookie")&&await h()===1&&await callBackendApi("/api/users/noCommissionCoupon",{method:"PUT"})}async function N(){let e=JSON.parse(sessionStorage.getItem("itemToBeCreatedAfterSignIn"));sessionStorage.removeItem("itemToBeCreatedAfterSignIn"),sessionStorage.removeItem("newItemId"),await callBackendApi(`/api/items/${e.id}`,{data:e}),await p(),await T(),localStorage.removeItem("newItem"),e.item.id=e.id,localStorage.setItem("latestItemCreated",JSON.stringify(e.item))}let D=()=>!(localStorage.getItem("latestItemCreated")||u||m.id);function F(){if(!D())return;let{user:e,createdAt:t,status:n,shippingStatus:a,...o}=M(),r=Object.keys(o).reduce((e,t)=>(e[t]=""===o[t]||jQuery.isEmptyObject(o[t])?null:o[t],e),{});r.defects=r.defects?r.defects:[],r.userValuationApproval=!0,delete r.preferences,r.acceptPrice=r.acceptPrice&&r.acceptPrice>0?r.acceptPrice:null,r.originalPrice=r.originalPrice&&r.originalPrice>0?r.originalPrice:null,["itemBrand","itemSize","itemMaterial","itemColor"].forEach(e=>{let t=document.getElementById(e).parentNode.querySelector(".suggest-buttons")||document.getElementById(e).parentNode.parentNode.querySelector(".suggest-buttons");t?.style?.display==="block"&&(r[`${e}Confirm`]=!0)}),R(r)?localStorage.removeItem("newItem"):localStorage.setItem("newItem",JSON.stringify(r))}function R(e){return function(e,t){for(let n in t)if(n in e){if(t[n]instanceof Object){if(JSON.stringify(t[n])!==JSON.stringify(e[n]))return!1;continue}if(e[n]!==t[n])return!1}return!0}(e,w())}function O(e,t,n){if(t&&n){let t=document.getElementById(e).parentNode.querySelector(".suggest-buttons")||document.getElementById(e).parentNode.parentNode.querySelector(".suggest-buttons");t.style.display="block",document.getElementById(e).setCustomValidity("Bekr\xe4fta eller \xe4ndra v\xe4rdet")}}async function H(e,t=null,n=!1){try{let o={data:t},i=null;t||([o,i]=await Promise.all([callBackendApi(`/api/items/${e}`),callBackendApi(`/api/items/${e}/atItem`)]),a=o);let l=i?.data||{},s=o.data,d=s.images||{},u=s.originalPrice;// Populate images
for(let e in u<=0&&(u=null),d){let t=d[`${e}Small`]||d[`${e}Medium`]||d[e]||d[`${e}Large`],n=d[e]||d[`${e}Large`]||d[`${e}Medium`]||d[`${e}Small`];y().includes(e)&&((0,r.rememberNewItemImageField)(e,n,t),"frontImage"===e&&(d.enhancedFrontImage?(t=d.enhancedFrontImageSmall||d.enhancedFrontImageMedium||d.enhancedFrontImage||d.enhancedFrontImageLarge,n=d.enhancedFrontImage||d.enhancedFrontImageLarge||d.enhancedFrontImageMedium||d.enhancedFrontImageSmall,(0,r.rememberNewItemImageField)("enhancedFrontImage",n,t)):(function(e){let t=new MutationObserver(e),n=document.getElementById("loadingDiv");t.observe(n,{attributeFilter:["style"]})}(()=>(0,r.showLoadingIcon)(e)),// Don't await here to don't block the form from showing with the front image
...
itemBrand.value=s.brand||"",s.brand&&(0,c.displayFindModelDiv)(s.brand).then(()=>{sessionStorage.getItem("models")&&s.modelVariantFields&&(0,c.showSelectedModel)(s.modelVariantFields)}),O("itemBrand",n,s.itemBrandConfirm),// Don't use the setFieldValue for the brand since that triggers a dropdown to open
...
t.exports=function(){return"function"==typeof Promise&&Promise.prototype&&Promise.prototype.then}},{}],e9qY0:[function(e,t,n){let a=e("4cf6a8173d9f3a2"),o=e("2ad62f61c352884c"),r=e("87d5a6270eb1dc26"),i=e("91abc94f777368cc"),l=e("9737c3939ab85d95"),s=e("cee3d371e219e45e"),d=e("8700c8c682afabf3"),c=e("65ad903a6ba3e"),u=e("1e8e447afb4d169c"),m=e("8a4a19af97836d80"),g=e("26720f9d94c9e268"),f=e("7b6429a248ecc51f"),y=e("1368d0fa14524351");/**
 * Add format info bits to matrix
 *
 * @param  {BitMatrix} matrix               Modules matrix
 * @param  {ErrorCorrectionLevel}    errorCorrectionLevel Error correction level
 * @param  {Number}    maskPattern          Mask pattern reference value
 */function p(e,t,n){let a,o;let r=e.size,i=g.getEncodedBits(t,n);for(a=0;a<15;a++)o=(i>>a&1)==1,a<6?e.set(a,8,o,!0):a<8?e.set(a+1,8,o,!0):e.set(r-15+a,8,o,!0),a<8?e.set(8,r-a-1,o,!0):a<9?e.set(8,15-a-1+1,o,!0):e.set(8,15-a-1,o,!0);// fixed module
e.set(r-8,8,1,!0)}/**
 * QR Code
 *
 ...
 */function(e,t,n,o){let g;if(Array.isArray(e))g=y.fromArray(e);else if("string"==typeof e){let a=t;if(!a){let t=y.rawSplit(e);// Estimate best version that can contain raw splitted segments
a=m.getBestVersionForData(t,n)}// Build optimized segments
// If estimated version is undefined, try with the highest version
g=y.fromString(e,a||40)}else throw Error("Invalid data");// Get the min version that can contain data
...
let i=a.getSymbolTotalCodewords(t),l=c.getTotalCodewordsCount(t,n),s=i-l,d=c.getBlocksCount(t,n),m=i%d,g=d-m,f=Math.floor(i/d),y=Math.floor(s/d),p=y+1,h=f-y,I=new u(h),S=0,B=Array(d),v=Array(d),x=0,E=new Uint8Array(e.buffer);// Divide the buffer into the required number of blocks
for(let e=0;e<d;e++){let t=e<g?y:p;// extract a block of data from buffer
B[e]=E.slice(S,S+t),// Calculate EC codewords for this data block
v[e]=I.encode(B[e]),S+=t,x=Math.max(x,t)}// Create final data
// Interleave the data and error correction codewords from each block
let b=new Uint8Array(i),k=0;// Add data codewords
for(o=0;o<x;o++)for(r=0;r<d;r++)o<B[r].length&&(b[k++]=B[r][o]);// Apped EC codewords
for(o=0;o<h;o++)for(r=0;r<d;r++)b[k++]=v[r][o];return b}(o,e,t))}(t,n,g),S=a.getSymbolSize(t),B=new i(S);return(// Add function modules
/**
 ...
 */function y(e,t){let n;let s=a.getBestModeForData(e);// Make sure data can be encoded
 ...
 */n.fromArray=function(e){return e.reduce(function(e,t){return"string"==typeof t?e.push(y(t,null)):t.data&&e.push(y(t.data,t.mode)),e},[])},/**
