...
var u=d(n);// CommonJS
"object"==typeof exports&&"undefined"!=typeof module?module.exports=u:"function"==typeof define&&define.amd?define(function(){return u}):o&&(this[o]=u)}}({"21npA":[function(e,t,n){let i,o,r,s;var l=e("@parcel/transformer-js/src/esmodule-helpers.js");l.defineInteropFlag(n),l.export(n,"loadInfoRequests",()=>g);var a=e("./general");/**
 ...
 */function d(e){return new Date(e&&"object"==typeof e&&void 0!==e._seconds?1e3*e._seconds+(e._nanoseconds||0)/1e6:e)}async function c(e,t){measurementDescriptionText.innerHTML=t,measurementsSubmitButton.addEventListener("click",async function(){let t=measurementsInput.value;t.length>0&&" "!==t&&await db.collection("items").doc(e).update({measurements:t,"infoRequests.measurements.status":"Resolved"}),triggerMeasurementsToastClose.click(),setTimeout(function(){location.reload()},400)}),triggerMeasurementsToastOpen.click()}async function u(e,t,n,i){if(!e)return;console.log("is this happening?");let o=10*Math.ceil(.7*n/10),r=10*Math.ceil(.8*n/10);priceAfterDiscount30.innerHTML=`(Priset blir ${o} kr)`,priceAfterDiscount20.innerHTML=`(Priset blir ${r} kr)`,priceNoDiscount.innerHTML=`(${n} kr)`,n>=140&&!i?(longerPeriodDescriptionText.innerHTML=`S\xe4ljperioden f\xf6r ditt ${t}-plagg har n\xe5tt sitt slut. V\xe4lj om du vill f\xf6rl\xe4nga s\xe4ljperioden med ytterligare 30 dagar och om du vill s\xe4nka priset f\xf6r att \xf6ka chansen att f\xe5 det s\xe5lt.`,discountFormDiv.style.display="block"):(longerPeriodDescriptionText.innerHTML=`S\xe4ljperioden f\xf6r ditt ${t}-plagg har n\xe5tt sitt slut. Vill du f\xf6rl\xe4nga s\xe4ljperioden med ytterligare 30 dagar eller avsluta f\xf6rs\xe4ljningen?`,discountFormDiv.style.display="none"),// Accept longer selling window and store chosen discount
longerPeriodAcceptButton.addEventListener("click",async function(){let t=new Date,i=t.toISOString().split("T")[0],s=n,l=0;for(var a=document.getElementsByName("Discount"),d=0;d<a.length;d++)if(a[d].checked){let e=a[d].value;"30"===e&&(s=o,l=30),"20"===e&&(s=r,l=20)}await db.collection("items").doc(e).update({longerPeriodAcceptedDate:i,"infoRequests.longerPeriod.status":"Resolved","infoRequests.longerPeriod.response":"Accepted",longerPeriodAcceptedDiscount:l,minPriceEstimate:s}),triggerLongerPeriodToastClose.click(),setTimeout(function(){location.reload()},300)}),// Decline longer selling period and quit sales
...
let o=new Date,l=d(i),c=l.getTime()-o.getTime(),u=Math.floor(c/6e4);bidDescription.innerHTML=`Budet ligger under ditt l\xe4gsta pris p\xe5 ${t} kr. Giltigt i ${Math.floor(u/60)} tim och ${Math.floor(u%60)} min.`,// Accept bid and show confirmation
bidAcceptButton.removeEventListener("click",r);let m=async function(){try{acceptBidLoading.style.display="block",acceptBidButtonText.style.display="none",// Call the API endpoint to accept the bid
...
}};bidAcceptButton.addEventListener("click",m),r=m,// Decline bid
...
(0,a.hideInfoRequestCard)(`infoRequestBid-${e}`)};bidDenyButton.addEventListener("click",p),s=p,(0,a.animateOpenToast)("bidToast")}async function p(e,t,n,i,o){// Deny price
if(console.log("storePriceResponse",e,t,n,i),"Accepted"===i&&await db.collection("items").doc(e).update({"infoRequests.price.status":"Resolved","infoRequests.price.response":"Accepted",maxPriceEstimate:t,minPriceEstimate:n}).then(function(){triggerNewPriceToastClose.click(),setTimeout(function(){location.reload()},300)}),"Denied"===i){let t={"infoRequests.price.status":"Resolved","infoRequests.price.response":"Denied"};"New"===o&&(t.archived=!0,t.willNotSell=!0),await db.collection("items").doc(e).update(t).then(function(){triggerNewPriceToastClose.click(),setTimeout(function(){location.reload()},300)})}}async function f(e,t,n,r,s,l,a,d,c,u){console.log("openNewPriceToast",e,t,n,r,s,l,a,d,c,u),previousMinPrice.style.display="none",previousMaxPrice.style.display="none",maxPriceDiv.style.display="block",minPriceDiv.style.display="block",// Set content of toast
newPriceToastTitle.innerHTML="Nytt l\xe4gsta pris",newPriceHeading.innerHTML=`${s}-plagg`;let m=a.toLowerCase();m&&"null"!==m&&(newPriceHeading.innerHTML=`${s}-${m}`),maxPrice.innerHTML=n,minPrice.innerHTML=r,c&&"null"!==c&&""!==c&&"undefined"!==c&&n!==c&&(previousMaxPrice.innerHTML=c,previousMaxPrice.style.display="block"),u&&"null"!==u&&""!==u&&"undefined"!==u&&r!==u&&(previousMinPrice.innerHTML=u,previousMinPrice.style.display="block"),acceptNewPriceButton.innerHTML="S\xe4lj med nytt pris",denyNewPriceButton.innerHTML="S\xe4nk ej","New"===t&&"Valuation"===d&&(newPriceToastTitle.innerHTML="V\xe4rdering",acceptNewPriceButton.innerHTML="S\xe4lj till v\xe4rdering",denyNewPriceButton.innerHTML="Avb\xf6j och avsluta"),"Adjusted ML Valuation"===d&&(newPriceToastTitle.innerHTML="Nytt prisintervall"),"Valuation"!==d&&"Adjusted ML Valuation"!==d&&(minPrice.innerHTML=`${r} kr`,maxPriceDiv.style.display="none"),l&&"undefined"!==l&&""!==l&&"null"!==l&&(newPriceText.innerHTML=l,descriptionDiv.style.display="block"),acceptNewPriceButton.removeEventListener("click",i);let f=()=>{p(e,n,r,"Accepted",t)};acceptNewPriceButton.addEventListener("click",f),i=f,denyNewPriceButton.removeEventListener("click",o);let g=()=>{p(e,n,r,"Denied",t)};denyNewPriceButton.addEventListener("click",g),o=g,// Open toast
triggerNewPriceToastOpen.click()}function g(e){console.log("loadInfoRequests");let t=document.getElementById("infoRequestBidTemplate").cloneNode(!0),n=document.getElementById("infoRequestMeasurementsTemplate").cloneNode(!0),i=document.getElementById("infoRequestLongerPeriodTemplate").cloneNode(!0),o=document.getElementById("infoRequestImagesTemplate").cloneNode(!0),r=document.getElementById("infoRequestValuationTemplate").cloneNode(!0),s=document.getElementById("infoRequestsList");s.replaceChildren(),(e||[]).forEach(e=>{let l=e.id,p=e.infoRequests,g=e.status,y=e.brand.replace(/'/g,""),v=e.minPriceEstimate,h=e.maxPriceEstimate,x=e?.infoRequests?.price?.response==="Denied",b=e.archived,I=e.category,E=(0,a.itemCoverImage)(e);void 0==b&&"Unsold"!==g&&"Sold"!==g&&p&&function(){for(let e in p)if(p[e]?.status==="Active"){let a=p[e].description;// PRICE REQUEST
if(a&&(a=a.replace(/'/g,"")),"price"===e){let t=p[e].type,n=r.cloneNode(!0);n.id=`infoRequestPrice-${l}`,n.querySelector(".img-container").style.backgroundImage=`url('${E}')`,n.querySelector("a .pricebuttontext").innerText="Se prisf\xf6rslag",n.querySelector(".text-block-72").innerText="Vill du s\xe4nka priset och f\xe5 det s\xe5lt?",s.appendChild(n),"New"===g&&"Adjusted ML Valuation"!==t?(n.querySelector("a .pricebuttontext").innerText="Se v\xe4rdering",n.querySelector(".text-block-72").innerText="Vill du s\xe4lja till v\xe5r v\xe4rdering?",n.querySelector("a").href=`/item-valuation?id=${l}`):setTimeout(()=>{let n=p[e].maxPrice,i=p[e].minPrice;document.querySelector(`#infoRequestPrice-${l} a`).addEventListener("click",async()=>{await f(l,g,n,i,y,a,I,t,h,v)})},0)}// MEASUREMENTS REQUEST
...
if("images"===e){let e=o.cloneNode(!0);e.id=`infoRequestImages-${l}`,e.querySelector(".img-container").style.backgroundImage=`url('${E}')`,e.querySelector("a").href=`/edit-item?id=${l}`,s.appendChild(e)}// LONGER PERIOD REQUEST
if("longerPeriod"===e){let e=i.cloneNode(!0);e.id=`infoRequestLongerPeriod-${l}`,e.querySelector(".img-container").style.backgroundImage=`url('${E}')`,s.appendChild(e),setTimeout(()=>{document.querySelector(`#infoRequestLongerPeriod-${l} a`).addEventListener("click",async()=>{await u(l,y,v,x)})},0)}// BID REQUEST
if("bid"===e){let n=p[e].expires,i=new Date,o=d(n);if(// Only show bid request if it hasn't expired
...
setTimeout(()=>{t.style.transform="translateY(0%)"},10),document.getElementById("darkOverlay").classList.add("active"))}function v(e){let t=document.getElementById(e);// Add the visibility check here
...
let e=document.getElementById("infoRequestsList");if(e){let t=e.querySelectorAll('[id^="infoRequest"]:not([style*="display: none"])');if(0===t.length){let e=document.getElementById("infoRequestsDiv");e&&(e.style.display="none")}}}}i.defineInteropFlag(n),i.export(n,"signOut",()=>o),i.export(n,"setFormAddressFields",()=>r),i.export(n,"getFormAddressFields",()=>s),// Validate Swedish Social Security Number (personnummer) using checksum
...
i.export(n,"closeMenuWithAnimation",()=>p),i.export(n,"prepareMenu",()=>f),i.export(n,"setupMenuHandlers",()=>g),// Toast animation functions
i.export(n,"animateOpenToast",()=>y),i.export(n,"animateCloseToast",()=>v),i.export(n,"hideInfoRequestCard",()=>h),// Make prepareMenu available globally for session cookie auto-login
