...
})({"b1RLQ":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = 50619;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "7474fe8b31666794";
"use strict";
...
},{}],"8CP9N":[function(require,module,exports) {
var _general = require("./general");
async function updateUserAddress() {
    try {
        loadOnSavePressed();
        await (0, _general.callBackendApi)("/api/users", {
            data: (0, _general.getFormAddressFields)()
        });
...
async function updateShippingPreference() {
    ...
    for(let x = 0; x < radioButtons.length; x++)if (radioButtons[x].checked) shippingMethod = radioButtons[x].value; // "Service point" or "Pickup"
    await (0, _general.callBackendApi)("/api/users", {
        data: {
            preferences: {
                shippingMethod
...
async function updateUserPersonId() {
    try {
        ...
        await (0, _general.callBackendApi)("/api/users", {
            data: {
                personalId: (0, _general.formatPersonalId)(personalId.value.trim().replace(/\D/g, ""))
            }
        });
        await onUpdateComplete();
    } catch (e) {
...
async function getUserInfo(onUpdate) {
    try {
        const response = await (0, _general.callBackendApi)("/api/users", {
            requiresAuth: true
        });
        data = response.data;
        ...
        if (shippingMethod) {
            if (shippingMethod === "Pickup") shippingPreferencesDisplay.innerHTML = "Upph\xe4mtning vid d\xf6rren";
            if (shippingMethod === "Service point") {
                shippingPreferencesDisplay.innerHTML = "L\xe4mna till ombud";
                // Hide expand button since we don't allow users to change to Pickup anymore
                expandShippingPrefButton.style.display = "none";
            }
        } else shippingPreferencesDisplay.innerHTML = "-";
    } catch (e) {
        errorHandler.report(e);
...
function getCleanedNumber(isSwish) {
    ...
    return isSwish ? {
        swishPayeeAlias: formatPhoneNumber(input)
    } : {
...
function displayForm(divId, formId, newValue, headerText) {
    ...
    if (newValue) {
        if (newValue === personalId) newValue.value = fullPersonId;
        else newValue.value = document.getElementById(divId).innerHTML.length > 2 ? document.getElementById(divId).innerHTML : "";
    ...
    if (formId === "shippingPrefForm") {
        ...
        if (document.getElementById(divId).innerHTML.includes("Upph\xe4mtning")) {
...
function isAllValid(ids) {
    let valid = true;
    for (const id of ids)if (!id.validity.valid) valid = false;
...
adressDiv.addEventListener("click", async function() {
    await (0, _general.setFormAddressFields)(data);
...
shippingPrefDiv.addEventListener("click", function() {
    if (shippingPreferencesDisplay.innerHTML.includes("ombud")) {
        ...
        setTimeout(function() {
            shippingPrefInfoText.style.display = "none";
        }, 3000);
    } else displayForm("shippingPreferencesDisplay", "shippingPrefForm", null, "Skicka plagg");
});
//Swish
...
saveButton.addEventListener("click", async function() {
    switch(pageHeader.value){
        ...
        case "Skicka plagg":
            ...
            break;
        case "Personnummer":
            if (isValid(personalId)) await updateUserPersonId();
            personalId.reportValidity();
            break;
        case "Swish":
            if (isValid(swishNumber)) await updateContactNumbers(true);
            swishNumber.reportValidity();
            break;
        case "Mobilnummer":
            if (isValid(phoneNumber)) await updateContactNumbers();
...
resetPageButton.addEventListener("click", ()=>{
    ...
    saveLoadingSpinner.style.display = "none";
    savedCheckIcon.style.display = "none";
    pageHeader.innerHTML = "Inst\xe4llningar";
    pageHeader.value = "Inst\xe4llningar";
});
getUserInfo();
...
},{}]},["b1RLQ","8CP9N"], "8CP9N", "parcelRequire81ca")
