...
})({"2ihIe":[function(require,module,exports) {
...
function getEventComponent(event, style) {
    ...
    if (event.type === "priceAdjusted") {
        ...
        return eventComponentHtml(displayLine, icon, className, `Pris sänktes till ${newPrice} kr ${platform && platform !== "" ? " p\xe5 " + capPlatform : ""}`, time);
    }
    ...
    if (event.type === "platformAdded") return (event.data.platforms || []).filter((p)=>p !== "Google Shopping" && p !== "Instagram Shop" && p !== "Facebook Shop").map((p)=>eventComponentHtml(displayLine, icon, className, `Publicerades på ${p}${p === "Mai Shop" ? "<br>(Google Shopping, Instagram Shop, Facebook Shop)" : ""}`, time)).join("\n");
    ...
    if (event.type === "itemSent") return eventComponentHtml(displayLine, icon, className, `Plagget skickades iväg`, time);
...
},{}]},["2ihIe","elTz7"], "elTz7", "parcelRequire81ca")
