...
var u=d(n);// CommonJS
"object"==typeof exports&&"undefined"!=typeof module?module.exports=u:"function"==typeof define&&define.amd?define(function(){return u}):r&&(this[r]=u)}}({"42EGR":[function(e,t,n){var a,r=e("@parcel/transformer-js/src/esmodule-helpers.js");r.defineInteropFlag(n),r.export(n,"isNoBgImage",()=>Q);var o=e("./sellItemHelpers"),i=e("qrcode"),l=r.interopDefault(i),s=e("./general"),d=e("./autocomplete-brands"),c=e("./sellItemModelSearch");async function u(){// Grab values from form
...
$("#frontImage").off("change"),e.addEventListener("change",D,{capture:!0}),$("#brandTagImage").off("change"),t.addEventListener("change",F,{capture:!0}),$("#productImage").off("change"),n.addEventListener("change",R,{capture:!0}),$("#defectImage").off("change"),a.addEventListener("change",O,{capture:!0}),$("#materialTag").off("change"),r.addEventListener("change",H,{capture:!0}),$("#extraImage").off("change"),i.addEventListener("change",V,{capture:!0}),(0,c.setupModelSearchEventListeners)(),function(){let e=document.getElementById("itemColor");e.onchange=function(){""!==this.value?e.style.color="#333":e.style.color="#929292"};// Change font color of dropdown itemAge when user selects a value
let t=document.getElementById("itemAge");t.onchange=function(){""!==this.value?t.style.color="#333":t.style.color="#929292"}}(),itemBrand.addEventListener("input",A("itemBrandLabel")),itemBrand.addEventListener("input",K),itemModel.addEventListener("input",A("itemModelLabel")),itemSize.addEventListener("input",A("itemSizeLabel")),itemSize.addEventListener("input",K),itemMaterial.addEventListener("input",A("itemMaterialLabel")),itemMaterial.addEventListener("input",K),itemOriginalPrice.addEventListener("input",A("itemOriginalPriceLabel")),itemAge.addEventListener("input",A("itemAgeLabel")),itemCondition.addEventListener("input",A("itemConditionLabel")),itemColor.addEventListener("change",A("itemColorLabel")),itemColor.addEventListener("input",K),itemUserComment.addEventListener("input",A("userCommentLabel")),document.getElementById("addItemButton").addEventListener("click",()=>{document.getElementById("wf-form-Add-Item").reportValidity();let e=document.getElementById("wf-form-Add-Item").querySelectorAll(":invalid"),t=e?.[0];t&&t.getBoundingClientRect().height<=1&&(t.style.cssText="width:100% !important;height:100% !important;"),setTimeout(()=>{if(e.length>0){if(!function(e){"function"==typeof jQuery&&e instanceof jQuery&&(e=e[0]);let t=e.getBoundingClientRect();return t.top>=0&&t.left>=0&&t.bottom<=(window.innerHeight/2||document.documentElement.clientHeight)&&t.right<=(window.innerWidth||document.documentElement.clientWidth)}(t)){let e=t.getBoundingClientRect().top+window.scrollY-40;window.scrollTo({top:e,behavior:"smooth"})}document.getElementById("wf-form-Add-Item").reportValidity()}},300)}),addItemForm.addEventListener("submit",p),userAddressForm.addEventListener("submit",u),(0,d.autocomplete)(document.getElementById("itemBrand"),d.brands);// Hide/Show warning about difficulty to sell certain brands
let f=document.getElementById("itemBrand");document.getElementById("hardToSellDiv"),f.oninput=function(){shareSoldDiv.style.display="none",function(e){if(words.some(t=>e.toLowerCase().includes(t.toLowerCase())))return hardToSellText.innerHTML=`Vi s\xe4ljer i regel inte ${e}-plagg p\xe5 grund av f\xf6r l\xe5gt andrahandsv\xe4rde. Undantag kan finnas.`,hardToSellDiv.style.display="block";hardToSellDiv.style.display="none"}(this.value)},// Hide/Show extra fields for defects
itemCondition.onchange=function(){let e=this.value;"Anv\xe4nd, tecken p\xe5 slitage"==e?(defectInfoDiv.style.display="block",itemCondition.style.color="#333"):""==e?(defectInfoDiv.style.display="none",itemCondition.style.color="#929292"):(defectInfoDiv.style.display="none",itemCondition.style.color="#333")},personalId.addEventListener("input",()=>{let e=(0,s.isValidSwedishSsn)(personalId.value)?"":"Ogiltigt personnummer";personalId.setCustomValidity(e)}),await _(),await z(),await G(),await J(),await q(),function(){let e=new MutationObserver((e,t)=>{let n=e.find(e=>"style"===e.attributeName);n&&"none"===n.target.style.display&&w()});Array.from(document.querySelectorAll(".suggest-buttons")).forEach(t=>e.observe(t,{attributes:!0}))}(),function(){function e(e){let t=e.target;if(t instanceof Element){let e=b()[t.name];e!==t.value&&""!==t.value&&(document.getElementById("clearItemForm").style.display="block")}}document.getElementById("wf-form-Add-Item").addEventListener("input",e),document.querySelector("#wf-form-Add-Item select").addEventListener("change",e)}(),g().forEach(e=>{document.getElementById(`delete${(0,o.capitalizeFirstLetter)(e)}Icon`).addEventListener("click",()=>{Y(e)})}),document.getElementById("deleteFrontImageIcon").addEventListener("click",()=>{document.getElementById("frontImage").required=!0,Y("enhancedFrontImage")}),document.getElementById("deleteBrandTagImageIcon").addEventListener("click",()=>{document.getElementById("brandTagImage").required=!0}),document.getElementById("clearItemForm").addEventListener("click",W);let h=getParamsObject();if(h.id)// Fill form if the user comes from a prefill link (re-sell item)
sessionStorage.removeItem("newItemId"),localStorage.removeItem("newItem"),auth.onAuthStateChanged(function(e){e||(document.getElementById("maiIntro").style.display="block")}),document.getElementById("resellIntro").style.display="block",await P(h.id),document.getElementById("triggerShowSellItemContent").click();else if(sessionStorage.getItem("itemToBeCreatedAfterSignIn")&&document.referrer.includes("/sign-in"));else if(localStorage.getItem("newItem")&&!M(JSON.parse(localStorage.getItem("newItem")))){// Saved state from a previous visit to /sell-item - restore the data
...
let i=a.getSymbolTotalCodewords(t),l=c.getTotalCodewordsCount(t,n),s=i-l,d=c.getBlocksCount(t,n),m=i%d,g=d-m,f=Math.floor(i/d),h=Math.floor(s/d),p=h+1,y=f-h,I=new u(y),S=0,B=Array(d),v=Array(d),b=0,E=new Uint8Array(e.buffer);// Divide the buffer into the required number of blocks
for(let e=0;e<d;e++){let t=e<g?h:p;// extract a block of data from buffer
B[e]=E.slice(S,S+t),// Calculate EC codewords for this data block
v[e]=I.encode(B[e]),S+=t,b=Math.max(b,t)}// Create final data
// Interleave the data and error correction codewords from each block
let x=new Uint8Array(i),C=0;// Add data codewords
for(r=0;r<b;r++)for(o=0;o<d;o++)r<B[o].length&&(x[C++]=B[o][r]);// Apped EC codewords
for(r=0;r<y;r++)for(o=0;o<d;o++)x[C++]=v[o][r];return x}(r,e,t))}(t,n,g),S=a.getSymbolSize(t),B=new i(S);return(// Add function modules
/**
