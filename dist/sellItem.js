...
if(document.referrer.includes("/sign-in")){await N();let e=sessionStorage.getItem("shippingMethod");e&&await (0,s.callBackendApi)("/api/users",{data:{preferences:{shippingMethod:e}}});let t=user.current?.phoneNumber?.length;return location.href=t?"/item-confirmation":"/user-contact"}sessionStorage.removeItem("itemToBeCreatedAfterSignIn")}}async function S(){let e,t,n,o,i,f;let h=document.getElementById("qrCanvas");if(h&&(0,l.default).toCanvas(h,window.location.href,function(e){e&&console.error(e),console.log("success!")}),localStorage.removeItem("latestItemCreated"),sessionStorage.removeItem("itemValuation"),sessionStorage.getItem("itemToBeCreatedAfterSignIn")&&document.referrer.includes("/sign-in")){// ... if we are redirected here from the sign-in page and have a saved item that should be created
...
itemBrand.value=d.brand||"",d.brand&&(0,c.displayFindModelDiv)(d.brand).then(()=>{if(sessionStorage.getItem("models")&&d.atModelVariantId){let e=JSON.parse(sessionStorage.getItem("models")),t=e.find(e=>e.atVariantId===d.atModelVariantId);t&&(0,c.showSelectedModel)(JSON.stringify(t))}}),O("itemBrand",n,d.itemBrandConfirm),// Don't use the setFieldValue for the brand since that triggers a dropdown to open
...
let i=a.getSymbolTotalCodewords(t),l=c.getTotalCodewordsCount(t,n),s=i-l,d=c.getBlocksCount(t,n),m=i%d,g=d-m,f=Math.floor(i/d),y=Math.floor(s/d),h=y+1,p=f-y,I=new u(p),S=0,v=Array(d),B=Array(d),b=0,x=new Uint8Array(e.buffer);// Divide the buffer into the required number of blocks
for(let e=0;e<d;e++){let t=e<g?y:h;// extract a block of data from buffer
