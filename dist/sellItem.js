...
})({"fg90f":[function(require,module,exports) {
    ...
    const valuationData = {
        ...
        ...decline || needsHumanCheck(mlValuationData) ? {} : {
            ...
            ...userValuationApproval ? {} : {
                minPriceEstimate: newMinPriceEstimate || minPrice,
                maxPriceEstimate: newMaxPriceEstimate || maxPrice
            },
            infoRequests: {
                price: {
...
async function setValuationFromResellItem(resellItem, item, itemId) {
    ...
    if (sessionStorage.getItem("itemToBeCreatedAfterSignIn")) sessionStorage.setItem("itemToBeCreatedAfterSignIn", JSON.stringify({
        id: item.id,
        item: {
            ...item.item,
            ...valuationData
...
async function getAndSaveValuation(itemId, item) {
    ...
    const params = getParamsObject();
    if (params.id) {
        const getItemResponse = await firebase.app().functions("europe-west1").httpsCallable("getItem")({
            itemId: params.id
        });
...
function rememberUnsavedChanges() {
    ...
    const { user: user1, createdAt, status, shippingStatus, modelVariantFields, ...itemToSave } = collect();
...
async function fillForm(itemId, savedItem = null, restoreSavedState = false) {
    try {
        ...
        if (!savedItem) [item, atItemResponse] = await Promise.all([
            firebase.app().functions("europe-west1").httpsCallable("getItem")({
                itemId
            }),
...
function checkBlockedOrLowShareSoldBrand(brand, category) {
    ...
    if (BLOCKED_BRANDS.includes(brand.toLowerCase()) || !HIGH_VALUE_CATEGORY.includes(category?.toLowerCase()) && BLOCK_NON_HIGH_VALUE_CATEGORY.includes(brand.toLowerCase()) || LOW_VALUE_CATEGORY.includes(category?.toLowerCase()) && BLOCK_ONLY_LOW_VALUE_CATEGORY.includes(brand.toLowerCase())) {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"aDjZV":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["fg90f","edX6B"], "edX6B", "parcelRequire81ca")
