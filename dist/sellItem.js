...
if(document.referrer.includes("/sign-in")){await N();let e=sessionStorage.getItem("shippingMethod");e&&await callBackendApi("/api/users",{data:{preferences:{shippingMethod:e}},method:"PUT"});let t=user.current?.phoneNumber?.length;return location.href=t?"/item-confirmation":"/user-contact"}sessionStorage.removeItem("itemToBeCreatedAfterSignIn")}}async function S(){let e,t,n,o,i,f;let h=document.getElementById("qrCanvas");if(h&&(0,l.default).toCanvas(h,window.location.href,function(e){e&&console.error(e),console.log("success!")}),localStorage.removeItem("latestItemCreated"),sessionStorage.removeItem("itemValuation"),sessionStorage.getItem("itemToBeCreatedAfterSignIn")&&document.referrer.includes("/sign-in")){console.log(`showing spinner and waiting for item to be created when user is set ${user.current?.email}`),// ... if we are redirected here from the sign-in page and have a saved item that should be created
...
let i=a.getSymbolTotalCodewords(t),l=c.getTotalCodewordsCount(t,n),s=i-l,d=c.getBlocksCount(t,n),m=i%d,g=d-m,f=Math.floor(i/d),y=Math.floor(s/d),h=y+1,p=f-y,I=new u(p),S=0,v=Array(d),B=Array(d),x=0,b=new Uint8Array(e.buffer);// Divide the buffer into the required number of blocks
for(let e=0;e<d;e++){let t=e<g?y:h;// extract a block of data from buffer
v[e]=b.slice(S,S+t),// Calculate EC codewords for this data block
B[e]=I.encode(v[e]),S+=t,x=Math.max(x,t)}// Create final data
// Interleave the data and error correction codewords from each block
let E=new Uint8Array(i),k=0;// Add data codewords
for(o=0;o<x;o++)for(r=0;r<d;r++)o<v[r].length&&(E[k++]=v[r][o]);// Apped EC codewords
