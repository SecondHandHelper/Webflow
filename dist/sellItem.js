...
var u=d(n);// CommonJS
"object"==typeof exports&&"undefined"!=typeof module?module.exports=u:"function"==typeof define&&define.amd?define(function(){return u}):o&&(this[o]=u)}}({"42EGR":[function(e,t,n){let a,o;var r=e("@parcel/transformer-js/src/esmodule-helpers.js");r.defineInteropFlag(n),r.export(n,"isNoBgImage",()=>en);var i=e("./sellItemHelpers"),l=e("qrcode"),s=r.interopDefault(l),d=e("./general"),c=e("./autocomplete-brands"),u=e("./sellItemModelSearch");let m=!1,g=getParamsObject();async function f(){// Grab values from form
...
if(document.referrer.includes("/sign-in")){document.getElementById("loadingDiv").style.display="flex",document.getElementById("creatingItemText").style.display="block",await T();let e=sessionStorage.getItem("shippingMethod");e&&await firebase.app().functions("europe-west1").httpsCallable("updateFirebaseUser")({preferences:{shippingMethod:e}});let t=user.current?.phoneNumber?.length;return location.href=t?"/item-confirmation":"/user-contact"}sessionStorage.removeItem("itemToBeCreatedAfterSignIn")}}async function b(){let e,t,n,o,r,l;let p=document.getElementById("qrCanvas");p&&(0,s.default).toCanvas(p,window.location.href,function(e){e&&console.error(e),console.log("success!")}),localStorage.removeItem("latestItemCreated"),sessionStorage.removeItem("itemValuation"),e=document.getElementById("frontImage"),t=document.getElementById("brandTagImage"),n=document.getElementById("productImage"),o=document.getElementById("defectImage"),r=document.getElementById("materialTagImage"),l=document.getElementById("extraImage"),// display image when file has been selected
$("#frontImage").off("change"),e.addEventListener("change",V,{capture:!0}),$("#brandTagImage").off("change"),t.addEventListener("change",K,{capture:!0}),$("#productImage").off("change"),n.addEventListener("change",U,{capture:!0}),$("#defectImage").off("change"),o.addEventListener("change",q,{capture:!0}),$("#materialTag").off("change"),r.addEventListener("change",z,{capture:!0}),$("#extraImage").off("change"),l.addEventListener("change",J,{capture:!0}),(0,u.setupModelSearchEventListeners)(),function(){let e=document.getElementById("itemColor");e.onchange=function(){""!==this.value?e.style.color="#333":e.style.color="#929292"};// Change font color of dropdown itemAge when user selects a value
let t=document.getElementById("itemAge");t.onchange=function(){""!==this.value?t.style.color="#333":t.style.color="#929292"}}(),itemBrand.addEventListener("input",(0,i.fieldLabelToggle)("itemBrandLabel")),itemBrand.addEventListener("input",G),itemModel.addEventListener("input",(0,i.fieldLabelToggle)("itemModelLabel")),itemSize.addEventListener("input",(0,i.fieldLabelToggle)("itemSizeLabel")),itemSize.addEventListener("input",G),itemMaterial.addEventListener("input",(0,i.fieldLabelToggle)("itemMaterialLabel")),itemMaterial.addEventListener("input",G),itemOriginalPrice.addEventListener("input",(0,i.fieldLabelToggle)("itemOriginalPriceLabel")),itemAge.addEventListener("input",(0,i.fieldLabelToggle)("itemAgeLabel")),itemCondition.addEventListener("input",(0,i.fieldLabelToggle)("itemConditionLabel")),itemColor.addEventListener("change",(0,i.fieldLabelToggle)("itemColorLabel")),itemColor.addEventListener("input",G),itemUserComment.addEventListener("input",(0,i.fieldLabelToggle)("userCommentLabel")),document.getElementById("saveItemDraftButton").addEventListener("click",async()=>{document.getElementById("saveItemDraftButton").style.display="none",document.getElementById("saveDraftSpinner").style.display="flex";let e=sessionStorage.getItem("newItemId")||await (0,i.requestUniqueId)(),t=await A(e,"Draft");m=!0,a=t,document.getElementById("clearItemForm").style.display="none",document.getElementById("saveDraftSpinner").style.display="none",document.getElementById("darkOverlay").style.display="block";let n=t.images?.enhancedFrontImageSmall||t.images?.enhancedFrontImage||t.images?.frontImage;n?(document.getElementById("popUpImage").src=n,document.getElementById("popUpImageDiv").style.display="block",document.getElementById("popUpCheckmark").style.display="none"):(document.getElementById("popUpImage").src="",document.getElementById("popUpImageDiv").style.display="none",document.getElementById("popUpCheckmark").style.display="block"),document.getElementById("itemDraftSavedPopup").style.display="flex"}),document.getElementById("closeItemSavedPopup").addEventListener("click",()=>{document.getElementById("itemDraftSavedPopup").style.display="none",document.getElementById("darkOverlay").style.display="none"}),document.getElementById("popUpNewItem").addEventListener("click",()=>{ee(),location.href="/sell-item"}),document.getElementById("goToMyWardrobe").addEventListener("click",()=>{window.location.href="/private#wardrobe"}),document.getElementById("addItemButton").addEventListener("click",()=>{document.getElementById("wf-form-Add-Item").reportValidity();let e=document.getElementById("wf-form-Add-Item").querySelectorAll(":invalid"),t=e?.[0];t&&t.getBoundingClientRect().height<=1&&(t.style.cssText="width:100% !important;height:100% !important;"),setTimeout(()=>{if(e.length>0){if(!function(e){"function"==typeof jQuery&&e instanceof jQuery&&(e=e[0]);let t=e.getBoundingClientRect();return t.top>=0&&t.left>=0&&t.bottom<=(window.innerHeight/2||document.documentElement.clientHeight)&&t.right<=(window.innerWidth||document.documentElement.clientWidth)}(t)){let e=t.getBoundingClientRect().top+window.scrollY-40;window.scrollTo({top:e,behavior:"smooth"})}document.getElementById("wf-form-Add-Item").reportValidity()}},300)}),addItemForm.addEventListener("submit",()=>v()),userAddressForm.addEventListener("submit",f),(0,c.autocomplete)(document.getElementById("itemBrand"),c.brands);// Hide/Show warning about difficulty to sell certain brands
...
let e="Service point";return user.current?.preferences?.shippingMethod?e=user.current?.preferences?.shippingMethod:authUser.current?await firebase.app().functions("europe-west1").httpsCallable("updateFirebaseUser")({preferences:{shippingMethod:e}}):sessionStorage.setItem("shippingMethod",e),e}async function A(e,t="New"){let{modelCoverImageUrl:n,images:a,...o}=M(),r=await L();n&&(a.modelImage=n);let i=g.id&&"draft"!==g.type?{createdFromItem:g.id}:{},l="Draft"===t?{draftSource:Object.keys(i).length?"Mai purchase":"Sell item"}:{},s={...o,status:t,...l,shippingMethod:r,images:a,...i,version:"2"};if(authUser.current){let t=await firebase.app().functions("europe-west1").httpsCallable("createItem")({id:e,item:s});await h(),P(),localStorage.removeItem("newItem"),sessionStorage.removeItem("newItemId"),localStorage.setItem("latestItemCreated",JSON.stringify(t.data))}else sessionStorage.setItem("itemToBeCreatedAfterSignIn",JSON.stringify({id:e,item:s}));return{...s,id:e}}async function P(){await I()===0&&"noCommission"===getCookie("noCommissionCampaignCookie")&&db.collection("users").doc(authUser.current.uid).update({oneTimeCommissionFreeCoupon:!0})}async function T(){let e=JSON.parse(sessionStorage.getItem("itemToBeCreatedAfterSignIn"));sessionStorage.removeItem("itemToBeCreatedAfterSignIn"),sessionStorage.removeItem("newItemId"),await firebase.app().functions("europe-west1").httpsCallable("createItem")(e),await h(),await P(),localStorage.removeItem("newItem"),e.item.id=e.id,localStorage.setItem("latestItemCreated",JSON.stringify(e.item))}let N=()=>!(localStorage.getItem("latestItemCreated")||m||g.id);function D(){if(!N())return;let{user:e,createdAt:t,status:n,shippingStatus:a,modelVariantFields:o,...r}=M(),i=Object.keys(r).reduce((e,t)=>(e[t]=""===r[t]?null:r[t],e),{});i.defects=i.defects?i.defects:[],i.userValuationApproval=!0,delete i.preferences,i.acceptPrice=i.acceptPrice&&i.acceptPrice>0?i.acceptPrice:null,i.originalPrice=i.originalPrice&&i.originalPrice>0?i.originalPrice:null,["itemBrand","itemSize","itemMaterial","itemColor"].forEach(e=>{let t=document.getElementById(e).parentNode.querySelector(".suggest-buttons")||document.getElementById(e).parentNode.parentNode.querySelector(".suggest-buttons");t?.style?.display==="block"&&(i[`${e}Confirm`]=!0)}),F(i)?localStorage.removeItem("newItem"):localStorage.setItem("newItem",JSON.stringify(i))}function F(e){return function(e,t){for(let n in t)if(n in e){if(t[n]instanceof Object){if(JSON.stringify(t[n])!==JSON.stringify(e[n]))return!1;continue}if(e[n]!==t[n])return!1}return!0}(e,w())}function R(e,t,n){if(t&&n){let t=document.getElementById(e).parentNode.querySelector(".suggest-buttons")||document.getElementById(e).parentNode.parentNode.querySelector(".suggest-buttons");t.style.display="block",document.getElementById(e).setCustomValidity("Bekr\xe4fta eller \xe4ndra v\xe4rdet")}}async function O(e,t=null,n=!1){try{let o={data:t},r=null;t||([o,r]=await Promise.all([firebase.app().functions("europe-west1").httpsCallable("getItem")({itemId:e}),fetch(`https://getatitem-heypmjzjfq-ew.a.run.app?itemId=${e}`)]),a=o);let l=await r?.json()||{},s=o.data,d=s.images||{},c=s.originalPrice;// Populate images
...
for(o=0;o<h;o++)for(r=0;r<d;r++)E[k++]=v[r][o];return E}(o,e,t))}(t,n,g),S=a.getSymbolSize(t),b=new i(S);return(// Add function modules
/**
