...
var u=d(n);// CommonJS
"object"==typeof exports&&"undefined"!=typeof module?module.exports=u:"function"==typeof define&&define.amd?define(function(){return u}):o&&(this[o]=u)}}({"42EGR":[function(e,t,n){var a,o=e("@parcel/transformer-js/src/esmodule-helpers.js");o.defineInteropFlag(n),o.export(n,"isNoBgImage",()=>_);var r=e("./sellItemHelpers"),i=e("qrcode"),l=o.interopDefault(i),s=e("./general"),d=e("./autocomplete-brands"),c=e("./sellItemModelSearch");let u=!1;async function m(){// Grab values from form
...
let n=db.collection("users").doc(authUser.current.uid);n.update({...e,personalId:t}).then(()=>{console.log(`User address of ${authUser.current.uid} is now updated`),itemConfirmationScreen.style.display="block",addressFormDiv.style.display="none"}).catch(e=>{errorHandler.report(e),console.error("Error updating document: ",e)})}function g(){return new Map().set("hole","H\xe5l").set("stain","Fl\xe4ck").set("lostFit","Tappad passform").set("nopprig","Nopprig").set("threadUp","Tr\xe5dsl\xe4pp").set("colorChange","F\xe4rg\xe4ndring").set("otherDefect","Annat")}function f(){return["frontImage","brandTagImage","defectImage","materialTagImage","extraImage"]}async function p(){console.log("sellItemMainAuthenticated "+new Date);let e=getParamsObject();// Create item from sessionStorage
if("draft"!==e.type&&(document.getElementById("saveItemDraftButton").style.display="flex"),// Visa alla "viktiga" fält om man är inloggad
...
if(document.referrer.includes("/sign-in")){document.getElementById("loadingDiv").style.display="flex",document.getElementById("creatingItemText").style.display="block",await w();let e=sessionStorage.getItem("shippingMethod");e&&await firebase.app().functions("europe-west1").httpsCallable("updateFirebaseUser")({preferences:{shippingMethod:e}});let t=user.current?.phoneNumber?.length;return location.href=t?"/item-confirmation":"/user-contact"}sessionStorage.removeItem("itemToBeCreatedAfterSignIn")}// Get user's item count to be able to send 'User Activated' event
let t=await getItems(authUser.current.uid);a=t.docs.filter(e=>e.data()?.status!=="Draft").length}async function h(){let e,t,n,a,o,i;let g=document.getElementById("qrCanvas");g&&(0,l.default).toCanvas(g,window.location.href,function(e){e&&console.error(e),console.log("success!")}),localStorage.removeItem("latestItemCreated"),sessionStorage.removeItem("itemValuation"),e=document.getElementById("frontImage"),t=document.getElementById("brandTagImage"),n=document.getElementById("productImage"),a=document.getElementById("defectImage"),o=document.getElementById("materialTagImage"),i=document.getElementById("extraImage"),// display image when file has been selected
$("#frontImage").off("change"),e.addEventListener("change",D,{capture:!0}),$("#brandTagImage").off("change"),t.addEventListener("change",F,{capture:!0}),$("#productImage").off("change"),n.addEventListener("change",R,{capture:!0}),$("#defectImage").off("change"),a.addEventListener("change",O,{capture:!0}),$("#materialTag").off("change"),o.addEventListener("change",H,{capture:!0}),$("#extraImage").off("change"),i.addEventListener("change",j,{capture:!0}),(0,c.setupModelSearchEventListeners)(),function(){let e=document.getElementById("itemColor");e.onchange=function(){""!==this.value?e.style.color="#333":e.style.color="#929292"};// Change font color of dropdown itemAge when user selects a value
let t=document.getElementById("itemAge");t.onchange=function(){""!==this.value?t.style.color="#333":t.style.color="#929292"}}(),itemBrand.addEventListener("input",(0,r.fieldLabelToggle)("itemBrandLabel")),itemBrand.addEventListener("input",V),itemModel.addEventListener("input",(0,r.fieldLabelToggle)("itemModelLabel")),itemSize.addEventListener("input",(0,r.fieldLabelToggle)("itemSizeLabel")),itemSize.addEventListener("input",V),itemMaterial.addEventListener("input",(0,r.fieldLabelToggle)("itemMaterialLabel")),itemMaterial.addEventListener("input",V),itemOriginalPrice.addEventListener("input",(0,r.fieldLabelToggle)("itemOriginalPriceLabel")),itemAge.addEventListener("input",(0,r.fieldLabelToggle)("itemAgeLabel")),itemCondition.addEventListener("input",(0,r.fieldLabelToggle)("itemConditionLabel")),itemColor.addEventListener("change",(0,r.fieldLabelToggle)("itemColorLabel")),itemColor.addEventListener("input",V),itemUserComment.addEventListener("input",(0,r.fieldLabelToggle)("userCommentLabel")),document.getElementById("saveItemDraftButton").addEventListener("click",async()=>{document.getElementById("saveItemDraftButton").style.display="none",document.getElementById("saveDraftSpinner").style.display="flex";let e=sessionStorage.getItem("newItemId")||await (0,r.requestUniqueId)(),t=await C(e,"Draft");u=!0,localStorage.removeItem("newItem"),document.getElementById("clearItemForm").style.display="none",document.getElementById("saveItemDraft").style.display="block",document.getElementById("saveDraftSpinner").style.display="none",document.getElementById("darkOverlay").style.display="block";let n=t.images.enhancedFrontImageSmall||t.images.enhancedFrontImage||t.images.frontImage;n?(document.getElementById("popUpImage").src=n,document.getElementById("popUpImageDiv").style.display="block",document.getElementById("popUpCheckmark").style.display="none"):(document.getElementById("popUpImage").src="",document.getElementById("popUpImageDiv").style.display="none",document.getElementById("popUpCheckmark").style.display="block"),document.getElementById("itemDraftSavedPopup").style.display="block"}),document.getElementById("closeItemSavedPopup").addEventListener("click",()=>{document.getElementById("itemDraftSavedPopup").style.display="none",document.getElementById("darkOverlay").style.display="none"}),document.getElementById("popUpNewItem").addEventListener("click",()=>{W(),document.getElementById("itemDraftSavedPopup").style.display="none",document.getElementById("darkOverlay").style.display="none",window.scrollTo(0,0),document.getElementById("bottomBarContainer").classList.remove("sticky-bottom-bar")}),document.getElementById("goToMyWardrobe").addEventListener("click",()=>{window.location.href="/private#wardrobe"}),document.getElementById("addItemButton").addEventListener("click",()=>{document.getElementById("wf-form-Add-Item").reportValidity();let e=document.getElementById("wf-form-Add-Item").querySelectorAll(":invalid"),t=e?.[0];t&&t.getBoundingClientRect().height<=1&&(t.style.cssText="width:100% !important;height:100% !important;"),setTimeout(()=>{if(e.length>0){if(!function(e){"function"==typeof jQuery&&e instanceof jQuery&&(e=e[0]);let t=e.getBoundingClientRect();return t.top>=0&&t.left>=0&&t.bottom<=(window.innerHeight/2||document.documentElement.clientHeight)&&t.right<=(window.innerWidth||document.documentElement.clientWidth)}(t)){let e=t.getBoundingClientRect().top+window.scrollY-40;window.scrollTo({top:e,behavior:"smooth"})}document.getElementById("wf-form-Add-Item").reportValidity()}},300)}),addItemForm.addEventListener("submit",()=>y()),userAddressForm.addEventListener("submit",m),(0,d.autocomplete)(document.getElementById("itemBrand"),d.brands);// Hide/Show warning about difficulty to sell certain brands
let p=document.getElementById("itemBrand"),h=document.getElementById("itemCategory");p.oninput=function(){shareSoldDiv.style.display="none",(0,r.checkBlockedOrLowShareSoldBrand)(this.value,h.value)},// Hide/Show extra fields for defects
itemCondition.onchange=function(){let e=this.value;"Anv\xe4nd, tecken p\xe5 slitage"==e?(defectInfoDiv.style.display="block",itemCondition.style.color="#333"):""==e?(defectInfoDiv.style.display="none",itemCondition.style.color="#929292"):(defectInfoDiv.style.display="none",itemCondition.style.color="#333")},personalId.addEventListener("input",()=>{let e=(0,s.isValidSwedishSsn)(personalId.value)?"":"Ogiltigt personnummer";personalId.setCustomValidity(e)}),(0,r.initializeCategorySelect)(),await G(),await q(),await z(),await J(),function(){let e=new MutationObserver((e,t)=>{let n=e.find(e=>"style"===e.attributeName);n&&"none"===n.target.style.display&&M()});Array.from(document.querySelectorAll(".suggest-buttons")).forEach(t=>e.observe(t,{attributes:!0}))}(),function(){function e(e){let t=e.target;if(t instanceof Element){let e=x()[t.name];e!==t.value&&""!==t.value&&(document.getElementById("clearItemForm").style.display="block")}}let t=getParamsObject();t.id||(document.getElementById("wf-form-Add-Item").addEventListener("input",e),document.querySelector("#wf-form-Add-Item select").addEventListener("change",e))}(),f().forEach(e=>{document.getElementById(`delete${(0,r.capitalizeFirstLetter)(e)}Icon`).addEventListener("click",()=>{Y(e)})}),document.getElementById("deleteFrontImageIcon").addEventListener("click",()=>{document.getElementById("frontImage").required=!0,Y("enhancedFrontImage")}),document.getElementById("deleteBrandTagImageIcon").addEventListener("click",()=>{document.getElementById("brandTagImage").required=!0}),document.getElementById("clearItemForm").addEventListener("click",W);let I=getParamsObject();if(window.addEventListener("scroll",function e(){window.innerHeight+window.pageYOffset>=document.body.offsetHeight-100&&(document.getElementById("bottomBarContainer").classList.add("sticky-bottom-bar"),window.removeEventListener("scroll",e))}),I.id){if(// Fill form if the user comes from a prefill link (re-sell item or continue with draft item)
sessionStorage.removeItem("newItemId"),localStorage.removeItem("newItem"),auth.onAuthStateChanged(function(e){e||(document.getElementById("maiIntro").style.display="block")}),document.getElementById("resellIntro").style.display="block",document.getElementById("bottomBarContainer").classList.add("sticky-bottom-bar"),document.getElementById("clearItemForm").style.display="none","draft"===I.type){document.querySelector("#resellIntro .text-block-176").innerText="Fyll i de sista detaljerna f\xf6r att s\xe4lja ditt plagg och kontrollera skickbeskrivningen.";let e=document.getElementById("saveItemDraftDiv");e.style.display="block",document.getElementById("saveItemDraft").addEventListener("click",async()=>{e.classList.add("saving"),await C(I.id,"Draft"),e.classList.remove("saving"),e.classList.add("saved"),setTimeout(()=>{e.classList.remove("saved")},1500)})}else document.getElementById("saveItemDraftButton").style.display="flex";await P(I.id,null,"draft"===I.type),document.getElementById("triggerShowSellItemContent").click()}else if(sessionStorage.getItem("itemToBeCreatedAfterSignIn")&&document.referrer.includes("/sign-in"));else if(localStorage.getItem("newItem")&&!L(JSON.parse(localStorage.getItem("newItem")))){// Saved state from a previous visit to /sell-item - restore the data
...
document.getElementById("wf-form-Add-Item").querySelectorAll("input").forEach(e=>{e.addEventListener("input",M)}),document.getElementById("wf-form-Add-Item").querySelectorAll('input[type="radio"]').forEach(e=>{e.addEventListener("change",M)}),document.getElementById("wf-form-Add-Item").querySelectorAll('input[type="checkbox"]').forEach(e=>{e.addEventListener("change",M)}),// We delay the rememberUnsavedChanges call to allow any confirm/reject animations to finish
document.getElementById("wf-form-Add-Item").querySelectorAll("select").forEach(e=>{e.addEventListener("change",M)}),document.getElementById("wf-form-Add-Item").querySelectorAll("textarea").forEach(e=>{e.addEventListener("input",M)}),window.addEventListener("pageshow",e=>{e.persisted&&localStorage.getItem("newItem")&&setTimeout(async()=>{await P(null,JSON.parse(localStorage.getItem("newItem")),!0)},10)})}async function y(){let e=sessionStorage.getItem("newItemId")||await (0,r.requestUniqueId)();try{document.getElementById("addItemFormDiv").style.display="none",document.getElementById("loadingDiv").style.display="flex",document.getElementById("clearItemForm").style.display="none";let t=await C(e),n=await B(e,t);0===a&&analytics.track("User Activated"),location.href=n}catch(e){errorHandler.report(e),console.error("addItem failed",e)}}function I({humanCheckNeeded:e,newMinMaxLog:t,lowValueSegment:n,lowValueCategory:a}){return e||t.match(/accept price is above max/i)&&!n&&!a}async function S(e,t){let{minPrice:n,maxPrice:a,decline:o,humanCheckNeeded:r,humanCheckExplanation:i,willNotSell:l,soldPrice:s,version:d,newMinPriceEstimate:c,newMaxPriceEstimate:u,newMinMaxLog:m,adjustmentAllowed:g,newBrand:f,newBrandCategory:p,valuatedBrandItems:h,brandMeanMax:y,brandAccuracy:S,brandCategoryAccuracy:b,fewBrand:B,brandMeanSold:v,brandCategoryMeanSold:x,highPriceVarBrandCategory:E,brandShareSold:k}=t||{};if(!n&&!o)return;let C={mlValuation:{decline:o,humanCheckNeeded:r,minPriceEstimate:n,maxPriceEstimate:a,humanCheckExplanation:i,willNotSellPrediction:l,soldPriceEstimate:s,modelVersion:d?.toString(),newMinPriceEstimate:c,newMaxPriceEstimate:u,newMinMaxLog:m,adjustmentAllowed:g,newBrand:f,newBrandCategory:p,valuatedBrandItems:h,brandMeanMax:y,brandAccuracy:S,brandCategoryAccuracy:b,fewBrand:B,brandMeanSold:v,brandCategoryMeanSold:x,highPriceVarBrandCategory:E,brandShareSold:k},...o||I(t)?{}:{valuationStatus:"Completed",valuationDate:new Date().toISOString(),infoRequests:{price:{status:"Active",response:"",description:"Vi b\xf6rjar med startpriset, och justerar successivt ner till l\xe4gsta priset under s\xe4ljperioden p\xe5 30 dagar. V\xe4rderingen utg\xe5r fr\xe5n vad liknande s\xe5lts f\xf6r.",minPrice:c||n,maxPrice:u||a}}}};if(sessionStorage.getItem("itemToBeCreatedAfterSignIn")){let e=JSON.parse(sessionStorage.getItem("itemToBeCreatedAfterSignIn"));sessionStorage.setItem("itemToBeCreatedAfterSignIn",JSON.stringify({id:e.id,item:{...e.item,...C}}))}else{await firebase.app().functions("europe-west1").httpsCallable("saveItemValuationFields")({itemId:e,...C});let t=JSON.parse(localStorage.getItem("latestItemCreated"));localStorage.setItem("latestItemCreated",JSON.stringify({...t,...C}))}}async function b(e,t,n){let a={valuationStatus:"Completed",valuationDate:new Date().toISOString(),infoRequests:{price:{status:"Active",response:"",description:"Vi b\xf6rjar med startpriset, och justerar successivt ner till l\xe4gsta priset under s\xe4ljperioden p\xe5 30 dagar. V\xe4rderingen utg\xe5r fr\xe5n vad liknande s\xe5lts f\xf6r.",minPrice:e.minPriceEstimate,maxPrice:e.maxPriceEstimate,type:"Valuation",source:"createdFromItem",adjustmentAllowed:!0}}};if(sessionStorage.getItem("itemToBeCreatedAfterSignIn"))sessionStorage.setItem("itemToBeCreatedAfterSignIn",JSON.stringify({id:t.id,item:{...t.item,...a}}));else{await firebase.app().functions("europe-west1").httpsCallable("saveItemValuationFields")({itemId:n,...a});let e=JSON.parse(localStorage.getItem("latestItemCreated"));localStorage.setItem("latestItemCreated",JSON.stringify({...e,...a}))}}async function B(e,t){if(!e&&!t)return console.error("No item and no itemId, unexpected!!"),"/item-confirmation";let n=getParamsObject();if(n.id){let a=await firebase.app().functions("europe-west1").httpsCallable("getItem")({itemId:n.id}),o=a.data;if("Sold"===o.status)return await b(o,t,e),"/item-valuation"}try{let n=await firebase.app().functions("europe-west1").httpsCallable("itemMlValuation")({itemId:e,item:t}),{minPrice:a,maxPrice:o,decline:r}=n.data||{};return await S(e,n.data),v(a&&o,r,I(n.data))}catch(e){console.error("Failed to get ml valuation",e)}return v()}function v(e,t,n){if(!e||n){if(sessionStorage.getItem("itemToBeCreatedAfterSignIn"))return"/sign-in";let e=user.current?.phoneNumber?.length;return e?"/item-confirmation":"/user-contact"}return"/item-valuation"}function x(){return{acceptPrice:null,age:null,brand:null,category:null,color:null,condition:null,defectDescription:null,defects:[],images:{},material:null,model:null,originalPrice:null,userValuationApproval:!0,sex:"Woman",size:null,userComment:null}}function E(){let e="",t=new Date,n=itemSize.value,a=itemMaterial.value?itemMaterial.value.trim():"",o=itemBrand.value?itemBrand.value.trim():"",r=itemModel.value?itemModel.value.trim():"",i=Number(itemOriginalPrice.value),l=itemAge.value,s=itemCondition.value,d=itemDefectDescription.value?itemDefectDescription.value.trim():"",c=itemUserComment.value?itemUserComment.value.trim():"",u=Number(itemLowestAcceptPrice.value),m=new Map().set("hole",hole.checked).set("stain",stain.checked).set("lostFit",lostFit.checked).set("nopprig",nopprig.checked).set("threadUp",threadUp.checked).set("colorChange",colorChange.checked).set("otherDefect",otherDefect.checked),f=[];"Anv\xe4nd, tecken p\xe5 slitage"===s&&m.forEach((e,t)=>{if(e){let e=g().get(t);f.push(e)}});for(var p=document.getElementsByName("Sex"),h=0;h<p.length;h++)p[h].checked&&(e=p[h].id);let y=document.getElementById("findModelBoxFilled"),I={};if("flex"===y.style.display){// There is a current model selected grab the cover image and id from it
let e=JSON.parse(y.getAttribute("data-model"));I={modelCoverImageUrl:e.coverImage,atModelVariantId:e.atVariantId}}let S=JSON.parse(localStorage.getItem("newItem")||"{}").images;return{user:authUser.current?.uid||null,createdAt:t.toISOString(),status,shippingStatus:"Not sent",sex:e,size:n,material:a,color:itemColor.value,category:itemCategory.value,brand:o,model:r,originalPrice:i,age:l,condition:s,defects:f,defectDescription:d,userComment:c,acceptPrice:u,preferences:{userValuationApproval:!0},...I,images:S}}async function k(){// If first time: User chooses shipping method preference in sell item form
let e="Service point";return user.current?.preferences?.shippingMethod?e=user.current?.preferences?.shippingMethod:authUser.current?await firebase.app().functions("europe-west1").httpsCallable("updateFirebaseUser")({preferences:{shippingMethod:e}}):sessionStorage.setItem("shippingMethod",e),e}async function C(e,t="New"){let{modelCoverImageUrl:n,images:a,...o}=E(),r=await k();n&&(a.modelImage=n);let i=getParamsObject(),l=i.id&&"draft"!==i.type?{createdFromItem:i.id}:{},s={...o,status:t,..."Draft"===t?{draftSource:"Sell item"}:{},shippingMethod:r,images:a,...l,version:"2"};if(authUser.current){let t=await firebase.app().functions("europe-west1").httpsCallable("createItem")({id:e,item:s});localStorage.removeItem("newItem"),sessionStorage.removeItem("newItemId"),localStorage.setItem("latestItemCreated",JSON.stringify(t.data))}else sessionStorage.setItem("itemToBeCreatedAfterSignIn",JSON.stringify({id:e,item:s}));return s}async function w(){let e=JSON.parse(sessionStorage.getItem("itemToBeCreatedAfterSignIn"));sessionStorage.removeItem("itemToBeCreatedAfterSignIn"),sessionStorage.removeItem("newItemId"),await firebase.app().functions("europe-west1").httpsCallable("createItem")(e),localStorage.removeItem("newItem"),e.item.id=e.id,localStorage.setItem("latestItemCreated",JSON.stringify(e.item))}function M(){let e=getParamsObject();if(localStorage.getItem("latestItemCreated")||u||e.id)return;let{user:t,createdAt:n,status:a,shippingStatus:o,modelVariantFields:r,...i}=E(),l=Object.keys(i).reduce((e,t)=>(e[t]=""===i[t]?null:i[t],e),{});l.defects=l.defects?l.defects:[],l.userValuationApproval=!0,delete l.preferences,l.acceptPrice=l.acceptPrice&&l.acceptPrice>0?l.acceptPrice:null,l.originalPrice=l.originalPrice&&l.originalPrice>0?l.originalPrice:null,["itemBrand","itemSize","itemMaterial","itemColor"].forEach(e=>{let t=document.getElementById(e).parentNode.querySelector(".suggest-buttons")||document.getElementById(e).parentNode.parentNode.querySelector(".suggest-buttons");t?.style?.display==="block"&&(l[`${e}Confirm`]=!0)}),L(l)?localStorage.removeItem("newItem"):localStorage.setItem("newItem",JSON.stringify(l))}function L(e){let t=x();for(let n in t)if(n in e){if(t[n]instanceof Object){if(JSON.stringify(t[n])!==JSON.stringify(e[n]))return!1;continue}if(e[n]!==t[n])return!1}return!0}function A(e,t,n){if(t&&n){let t=document.getElementById(e).parentNode.querySelector(".suggest-buttons")||document.getElementById(e).parentNode.parentNode.querySelector(".suggest-buttons");t.style.display="block",document.getElementById(e).setCustomValidity("Bekr\xe4fta eller \xe4ndra v\xe4rdet")}}async function P(e,t=null,n=!1){try{let a={data:t},o=null;t||([a,o]=await Promise.all([firebase.app().functions("europe-west1").httpsCallable("getItem")({itemId:e}),fetch(`https://getatitem-heypmjzjfq-ew.a.run.app?itemId=${e}`)]));let i=await o?.json()||{},l=a.data,s=l.images||{},d=l.originalPrice;// Populate images
for(let e in d<=0&&(d=null),s){let t=s[`${e}Small`]||s[`${e}Medium`]||s[e]||s[`${e}Large`],n=s[e]||s[`${e}Large`]||s[`${e}Medium`]||s[`${e}Small`];f().includes(e)&&((0,r.rememberNewItemImageField)(e,n,t),"frontImage"===e&&(s.enhancedFrontImage?(t=s.enhancedFrontImageSmall||s.enhancedFrontImageMedium||s.enhancedFrontImage||s.enhancedFrontImageLarge,n=s.enhancedFrontImage||s.enhancedFrontImageLarge||s.enhancedFrontImageMedium||s.enhancedFrontImageSmall,(0,r.rememberNewItemImageField)("enhancedFrontImage",n,t)):(function(e){let t=new MutationObserver(e),n=document.getElementById("loadingDiv");t.observe(n,{attributeFilter:["style"]})}(()=>(0,r.showLoadingIcon)(e)),// Don't await here to don't block the form from showing with the front image
(0,r.enhanceFrontImage)(n).then(()=>console.log("Image enhanced")))),(0,r.showImagePreview)(e,t),(0,r.showImageState)(e,"success-state"),document.getElementById(e).required=!1)}if(s.modelImage){let e=s.modelImageLarge||s.modelImage,t=s.modelImageSmall||s.modelImageMedium||s.modelImage;document.getElementById("coverImageContainer").style.backgroundImage=`url('${t}')`,document.getElementById("coverImagePreview").style.display="block",(0,r.rememberNewItemImageField)("modelImage",e,t)}else if(s.coverImage&&!await _(s.coverImage)){// Show cover image preview if it is a model image, if it is a noBg image we skip it
...
let i=a.getSymbolTotalCodewords(t),l=c.getTotalCodewordsCount(t,n),s=i-l,d=c.getBlocksCount(t,n),m=i%d,g=d-m,f=Math.floor(i/d),p=Math.floor(s/d),h=p+1,y=f-p,I=new u(y),S=0,b=Array(d),B=Array(d),v=0,x=new Uint8Array(e.buffer);// Divide the buffer into the required number of blocks
...
for(o=0;o<v;o++)for(r=0;r<d;r++)o<b[r].length&&(E[k++]=b[r][o]);// Apped EC codewords
for(o=0;o<y;o++)for(r=0;r<d;r++)E[k++]=B[r][o];return E}(o,e,t))}(t,n,g),S=a.getSymbolSize(t),b=new i(S);return(// Add function modules
/**
