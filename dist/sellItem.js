...
if(document.referrer.includes("/sign-in")){document.getElementById("loadingDiv").style.display="flex",document.getElementById("creatingItemText").style.display="block",await N();let e=sessionStorage.getItem("shippingMethod");e&&await (0,s.callFirebaseFunction)("europe-west1","updateFirebaseUser",{preferences:{shippingMethod:e}});let t=user.current?.phoneNumber?.length;return location.href=t?"/item-confirmation":"/user-contact"}sessionStorage.removeItem("itemToBeCreatedAfterSignIn")}}async function S(){let e,t,n,o,i,f;let y=document.getElementById("qrCanvas");y&&(0,l.default).toCanvas(y,window.location.href,function(e){e&&console.error(e),console.log("success!")}),localStorage.removeItem("latestItemCreated"),sessionStorage.removeItem("itemValuation"),e=document.getElementById("frontImage"),t=document.getElementById("brandTagImage"),n=document.getElementById("productImage"),o=document.getElementById("defectImage"),i=document.getElementById("materialTagImage"),f=document.getElementById("extraImage"),// display image when file has been selected
...
let p=document.getElementById("itemBrand"),I=document.getElementById("itemCategory");if(p.oninput=function(){shareSoldDiv.style.display="none",(0,r.checkBlockedOrLowShareSoldBrand)(this.value,I.value)},p.onblur=function(){let e=document.getElementById("hardToSellDiv");(["Ok\xe4nt","Unknown","Vet ej","Vet inte","Ok\xe4nd","Se bild"].some(e=>this.value.toLowerCase().includes(e.toLowerCase()))||this.value.length&&!this.value.match(/(\w|\d)/))&&(hardToSellText.innerHTML=`Vi k\xe4nner inte till m\xe4rket '${this.value}', och s\xe4ljer i regel inte ok\xe4nda varum\xe4rken.`,stopIcon.style.display="none",warningIcon.style.display="block",e.style.display="block")},// Hide/Show extra fields for defects
 ...
 */n.create=function(e,t){let n,g;if(void 0===e||""===e)throw Error("No input text");let p=o.M;return void 0!==t&&(// Use higher error correction level as default
p=o.from(t.errorCorrectionLevel,o.M),n=m.from(t.version),g=d.from(t.maskPattern),t.toSJISFunc&&a.setToSJISFunction(t.toSJISFunc)),/**
 ...
 */function(e,t,n,o){let g;if(Array.isArray(e))g=h.fromArray(e);else if("string"==typeof e){let a=t;if(!a){let t=h.rawSplit(e);// Estimate best version that can contain raw splitted segments
...
let i=a.getSymbolTotalCodewords(t),l=c.getTotalCodewordsCount(t,n),s=i-l,d=c.getBlocksCount(t,n),m=i%d,g=d-m,f=Math.floor(i/d),h=Math.floor(s/d),y=h+1,p=f-h,I=new u(p),S=0,v=Array(d),b=Array(d),B=0,x=new Uint8Array(e.buffer);// Divide the buffer into the required number of blocks
for(let e=0;e<d;e++){let t=e<g?h:y;// extract a block of data from buffer
...
for(o=0;o<p;o++)for(r=0;r<d;r++)E[k++]=b[r][o];return E}(o,e,t))}(t,n,g),S=a.getSymbolSize(t),v=new i(S);return(// Add function modules
/**
