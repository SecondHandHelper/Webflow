...
document.getElementById("wf-form-Add-Item").querySelectorAll("select").forEach(e=>{e.addEventListener("change",w)}),document.getElementById("wf-form-Add-Item").querySelectorAll("textarea").forEach(e=>{e.addEventListener("input",w)}),window.addEventListener("pageshow",e=>{e.persisted&&localStorage.getItem("newItem")&&setTimeout(async()=>{await A(null,JSON.parse(localStorage.getItem("newItem")),!0)},10)})}async function p(){let e=sessionStorage.getItem("newItemId")||await (0,o.requestUniqueId)();try{document.getElementById("addItemFormDiv").style.display="none",document.getElementById("loadingDiv").style.display="flex",document.getElementById("clearItemForm").style.display="none";let t=await k(e),n=await b(e,t);0===a&&analytics.track("User Activated"),location.href=n}catch(e){errorHandler.report(e),console.error("addItem failed",e)}}function y({humanCheckNeeded:e,newMinMaxLog:t,lowValueSegment:n,lowValueCategory:a}){return e||t.match(/accept price is above max/i)&&!n&&!a}async function I(e,t){let{minPrice:n,maxPrice:a,decline:r,humanCheckNeeded:o,humanCheckExplanation:i,willNotSell:l,soldPrice:s,version:d,newMinPriceEstimate:c,newMaxPriceEstimate:u,newMinMaxLog:m,adjustmentAllowed:g,newBrand:f,newBrandCategory:h,valuatedBrandItems:p,brandMeanMax:I,brandAccuracy:S,brandCategoryAccuracy:b,fewBrand:v,brandMeanSold:B,brandCategoryMeanSold:x,highPriceVarBrandCategory:E,brandShareSold:k}=t||{};if(!n&&!r)return;let C={mlValuation:{decline:r,humanCheckNeeded:o,minPriceEstimate:n,maxPriceEstimate:a,humanCheckExplanation:i,willNotSellPrediction:l,soldPriceEstimate:s,modelVersion:d?.toString(),newMinPriceEstimate:c,newMaxPriceEstimate:u,newMinMaxLog:m,adjustmentAllowed:g,newBrand:f,newBrandCategory:h,valuatedBrandItems:p,brandMeanMax:I,brandAccuracy:S,brandCategoryAccuracy:b,fewBrand:v,brandMeanSold:B,brandCategoryMeanSold:x,highPriceVarBrandCategory:E,brandShareSold:k},...r||y(t)?{}:{valuationStatus:"Completed",valuationDate:new Date().toISOString(),infoRequests:{price:{status:"Active",response:"",description:"Vi b\xf6rjar med startpriset, och justerar successivt ner till l\xe4gsta priset under s\xe4ljperioden p\xe5 30 dagar. V\xe4rderingen utg\xe5r fr\xe5n vad liknande s\xe5lts f\xf6r.",minPrice:c||n,maxPrice:u||a}}}};if(sessionStorage.getItem("itemToBeCreatedAfterSignIn")){let e=JSON.parse(sessionStorage.getItem("itemToBeCreatedAfterSignIn"));sessionStorage.setItem("itemToBeCreatedAfterSignIn",JSON.stringify({id:e.id,item:{...e.item,...C}}))}else{await firebase.app().functions("europe-west1").httpsCallable("saveItemValuationFields")({itemId:e,...C});let t=JSON.parse(localStorage.getItem("latestItemCreated"));localStorage.setItem("latestItemCreated",JSON.stringify({...t,...C}))}}async function S(e,t,n){let a={valuationStatus:"Completed",valuationDate:new Date().toISOString(),infoRequests:{price:{status:"Active",response:"",description:"Vi b\xf6rjar med startpriset, och justerar successivt ner till l\xe4gsta priset under s\xe4ljperioden p\xe5 30 dagar. V\xe4rderingen utg\xe5r fr\xe5n vad liknande s\xe5lts f\xf6r.",minPrice:e.minPriceEstimate,maxPrice:e.maxPriceEstimate,type:"Valuation",source:"createdFromItem",adjustmentAllowed:!0}}};if(sessionStorage.getItem("itemToBeCreatedAfterSignIn"))sessionStorage.setItem("itemToBeCreatedAfterSignIn",JSON.stringify({id:t.id,item:{...t.item,...a}}));else{await firebase.app().functions("europe-west1").httpsCallable("saveItemValuationFields")({itemId:n,...a});let e=JSON.parse(localStorage.getItem("latestItemCreated"));localStorage.setItem("latestItemCreated",JSON.stringify({...e,...a}))}}async function b(e,t){if(!e&&!t)return console.error("No item and no itemId, unexpected!!"),"/item-confirmation";let n=getParamsObject();if(n.id){let a=await firebase.app().functions("europe-west1").httpsCallable("getItem")({itemId:n.id}),r=a.data;if("Sold"===r.status)return await S(r,t,e),"/item-valuation"}try{let n=await firebase.app().functions("europe-west1").httpsCallable("itemMlValuation")({itemId:e,item:t}),{minPrice:a,maxPrice:r,decline:o}=n.data||{};return await I(e,n.data),v(a&&r,o,y(n.data))}catch(e){console.error("Failed to get ml valuation",e)}return v()}function v(e,t,n){if(!e||n){if(sessionStorage.getItem("itemToBeCreatedAfterSignIn"))return"/sign-in";let e=user.current?.phoneNumber?.length;return e?"/item-confirmation":"/user-contact"}return"/item-valuation"}function B(){return{acceptPrice:null,age:null,brand:null,category:null,color:null,condition:null,defectDescription:null,defects:[],images:{},material:null,model:null,originalPrice:null,userValuationApproval:!0,sex:"Woman",size:null,userComment:null}}function x(){let e="",t=new Date,n=itemSize.value,a=itemMaterial.value?itemMaterial.value.trim():"",r=itemBrand.value?itemBrand.value.trim():"",o=itemModel.value?itemModel.value.trim():"",i=Number(itemOriginalPrice.value),l=itemAge.value,s=itemCondition.value,d=itemDefectDescription.value?itemDefectDescription.value.trim():"",c=itemUserComment.value?itemUserComment.value.trim():"",u=Number(itemLowestAcceptPrice.value),g=new Map().set("hole",hole.checked).set("stain",stain.checked).set("lostFit",lostFit.checked).set("nopprig",nopprig.checked).set("threadUp",threadUp.checked).set("colorChange",colorChange.checked).set("otherDefect",otherDefect.checked),f=[];"Anv\xe4nd, tecken p\xe5 slitage"===s&&g.forEach((e,t)=>{if(e){let e=m().get(t);f.push(e)}});for(var h=document.getElementsByName("Sex"),p=0;p<h.length;p++)h[p].checked&&(e=h[p].id);let y=document.getElementById("findModelBoxFilled"),I={};if("flex"===y.style.display){// There is a current model selected grab the cover image and id from it
let e=JSON.parse(y.getAttribute("data-model"));I={modelCoverImageUrl:e.coverImage,atModelVariantId:e.atVariantId}}let S=JSON.parse(localStorage.getItem("newItem")||"{}").images;return{user:authUser.current?.uid||null,createdAt:t.toISOString(),status,shippingStatus:"Not sent",sex:e,size:n,material:a,color:itemColor.value,category:itemCategory.value,brand:r,model:o,originalPrice:i,age:l,condition:s,defects:f,defectDescription:d,userComment:c,acceptPrice:u,preferences:{userValuationApproval:!0},...I,images:S}}async function E(){// If first time: User chooses shipping method preference in sell item form
let e="Service point";return user.current?.preferences?.shippingMethod?e=user.current?.preferences?.shippingMethod:authUser.current?await firebase.app().functions("europe-west1").httpsCallable("updateFirebaseUser")({preferences:{shippingMethod:e}}):sessionStorage.setItem("shippingMethod",e),e}async function k(e,t="New"){let{modelCoverImageUrl:n,images:a,...r}=x(),o=await E();n&&(a.modelImage=n);let i=getParamsObject(),l=i.id&&"draft"!==i.type?{createdFromItem:i.id}:{},s={...r,status:t,..."Draft"===t?{draftSource:"S\xe4ljsidan"}:{},shippingMethod:o,images:a,...l,version:"2"};if(authUser.current){let t=await firebase.app().functions("europe-west1").httpsCallable("createItem")({id:e,item:s});localStorage.removeItem("newItem"),sessionStorage.removeItem("newItemId"),localStorage.setItem("latestItemCreated",JSON.stringify(t.data))}else sessionStorage.setItem("itemToBeCreatedAfterSignIn",JSON.stringify({id:e,item:s}));return s}async function C(){let e=JSON.parse(sessionStorage.getItem("itemToBeCreatedAfterSignIn"));sessionStorage.removeItem("itemToBeCreatedAfterSignIn"),sessionStorage.removeItem("newItemId"),await firebase.app().functions("europe-west1").httpsCallable("createItem")(e),localStorage.removeItem("newItem"),e.item.id=e.id,localStorage.setItem("latestItemCreated",JSON.stringify(e.item))}function w(){if(localStorage.getItem("latestItemCreated"))return;let{user:e,createdAt:t,status:n,shippingStatus:a,modelVariantFields:r,...o}=x(),i=Object.keys(o).reduce((e,t)=>(e[t]=""===o[t]?null:o[t],e),{});i.defects=i.defects?i.defects:[],i.userValuationApproval=!0,delete i.preferences,i.acceptPrice=i.acceptPrice&&i.acceptPrice>0?i.acceptPrice:null,i.originalPrice=i.originalPrice&&i.originalPrice>0?i.originalPrice:null,["itemBrand","itemSize","itemMaterial","itemColor"].forEach(e=>{let t=document.getElementById(e).parentNode.querySelector(".suggest-buttons")||document.getElementById(e).parentNode.parentNode.querySelector(".suggest-buttons");t?.style?.display==="block"&&(i[`${e}Confirm`]=!0)}),M(i)?localStorage.removeItem("newItem"):localStorage.setItem("newItem",JSON.stringify(i))}function M(e){let t=B();for(let n in t)if(n in e){if(t[n]instanceof Object){if(JSON.stringify(t[n])!==JSON.stringify(e[n]))return!1;continue}if(e[n]!==t[n])return!1}return!0}function L(e,t,n){if(t&&n){let t=document.getElementById(e).parentNode.querySelector(".suggest-buttons")||document.getElementById(e).parentNode.parentNode.querySelector(".suggest-buttons");t.style.display="block",document.getElementById(e).setCustomValidity("Bekr\xe4fta eller \xe4ndra v\xe4rdet")}}async function A(e,t=null,n=!1){try{let a={data:t},r=null;t||([a,r]=await Promise.all([firebase.app().functions("europe-west1").httpsCallable("getItem")({itemId:e}),fetch(`https://getatitem-heypmjzjfq-ew.a.run.app?itemId=${e}`)]));let i=await r?.json()||{},l=a.data,s=l.images||{},d=l.originalPrice;// Populate images
