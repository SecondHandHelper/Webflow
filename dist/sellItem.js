...
if(document.referrer.includes("/sign-in")){await N();let e=sessionStorage.getItem("shippingMethod");e&&await (0,s.callBackendApi)("/api/users",{data:{preferences:{shippingMethod:e}}});let t=user.current?.phoneNumber?.length;return location.href=t?"/item-confirmation":"/user-contact"}sessionStorage.removeItem("itemToBeCreatedAfterSignIn")}}async function S(){let e,t,n,o,i,f;console.log("sellItemMain");let y=document.getElementById("qrCanvas");if(y&&(0,l.default).toCanvas(y,window.location.href,function(e){e&&console.error(e),console.log("success!")}),localStorage.removeItem("latestItemCreated"),sessionStorage.removeItem("itemValuation"),sessionStorage.getItem("itemToBeCreatedAfterSignIn")&&document.referrer.includes("/sign-in")){console.log(`showing spinner and waiting for item to be created when user is set ${user.current?.email}`),// ... if we are redirected here from the sign-in page and have a saved item that should be created
...
a.export(n,"isValidSwedishSsn",()=>c),a.export(n,"formatPersonalId",()=>u),a.export(n,"itemCoverImage",()=>m),a.export(n,"shareCode",()=>g);let r="https://europe-west1-second-hand-helper.cloudfunctions.net/webApi";async function i(e,{data:t,method:n,requiresAuth:a=!0,timeoutSec:o=20}={}){// const { data, method, requiresAuth, timeoutSec = 20 } = opts;
