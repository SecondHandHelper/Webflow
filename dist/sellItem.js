...
if(document.referrer.includes("/sign-in")){await N();let e=sessionStorage.getItem("shippingMethod");e&&await (0,s.callBackendApi)("/api/users",{data:{preferences:{shippingMethod:e}}});let t=user.current?.phoneNumber?.length;return location.href=t?"/item-confirmation":"/user-contact"}sessionStorage.removeItem("itemToBeCreatedAfterSignIn")}}async function S(){let e,t,n,o,i,f;let p=document.getElementById("qrCanvas");if(p&&(0,l.default).toCanvas(p,window.location.href,function(e){e&&console.error(e),console.log("success!")}),localStorage.removeItem("latestItemCreated"),sessionStorage.removeItem("itemValuation"),sessionStorage.getItem("itemToBeCreatedAfterSignIn")&&document.referrer.includes("/sign-in")){// ... if we are redirected here from the sign-in page and have a saved item that should be created
...
let i=a.getSymbolTotalCodewords(t),l=c.getTotalCodewordsCount(t,n),s=i-l,d=c.getBlocksCount(t,n),m=i%d,g=d-m,f=Math.floor(i/d),y=Math.floor(s/d),p=y+1,h=f-y,I=new u(h),S=0,v=Array(d),B=Array(d),b=0,x=new Uint8Array(e.buffer);// Divide the buffer into the required number of blocks
