...
})({"vt3gJ":[function(require,module,exports,__globalThis) {
...
},{}],"9IZ5h":[function(require,module,exports,__globalThis) {
...
async function validateInput() {
    return new Promise((resolve, reject)=>{
        ...
        if (!isNumeric(currentPrice.value) || Number(currentPrice.value) > initialCurrentPrice) {
            document.getElementById('currentPrice').setCustomValidity(`Ange ett pris som \xe4r l\xe4gre \xe4n nuvarande pris p\xe5 ${initialCurrentPrice} kr`);
            document.getElementById('wf-form-Add-Item').reportValidity();
            showSaveButton();
            return resolve(false);
        }
        ...
        document.getElementById('lowerPriceConfirm').innerText = `Priset g\xe5r under intervallet p\xe5 ${lowestPrice}-${startPrice} kr. Vill du s\xe4nka priset \xe4nd\xe5?`;
        document.getElementById('confirmLowerPrice').innerText = `Ja, s\xe4nk priset till ${currentPrice.value} kr`;
...
async function updateItem(itemId, changedImages) {
    ...
    function inputNameToImageName(inputName, numExtraImagesSeen, numDefectImagesSeen) {
        ...
        if (inputName === 'brandTagImage') return 'extraImage1';
        else if (inputName === 'materialTagImage') return 'extraImage2';
        else if (inputName.includes('defectImage')) return `defectImage${numDefectImagesSeen + 1}`;
        else if (inputName.includes('extraImage')) return `extraImage${numExtraImagesSeen + 1}`;
    ...
    async function uploadImages(itemId) {
        ...
        if (changedImages.length >= 0) {
            ...
            ].find((input)=>input.value?.length) != null;
            if (newFilesUploaded) await db.collection("items").doc(itemId).get().then((doc)=>{
            ...
            if (imagesv2) {
                ...
                document.querySelectorAll('input[type=file]').forEach((fileInput)=>{
                    if (fileInput.value?.length || fileInput.dataset.fileUrl?.length) {
                        ...
                        if (imageName.includes('extraImage')) numExtraImagesSeen++;
                        else if (imageName.includes('defectImage')) numDefectImagesSeen++;
                    }
                });
            ...
            await Promise.all(Array.from(images).map(async ([imageName, value])=>{
                ...
                const { url: imageUrl } = value instanceof File ? await (0, _sellItemHelpers.uploadTempImage)(value, imageName, itemId) : {
                    url: value
                };
                if (!imagesv2) await callBackendApi(`/api/items/${itemId}/images`, {
                    data: {
                        fileName: imageName,
                        url: imageUrl
                    }
                });
                else changes['imagesv2'] = [
            ...
            if (changes['imagesv2']) changes['imagesv2Status'] = {
                status: 'Process',
    ...
    try {
        ...
        function showDefectImagesV2(defectImagesV2) {
            ...
            defectImagesV2.sort((a, b)=>a.name.localeCompare(b.name)).forEach((imageV2, idx)=>{
                ...
                (0, _sellItemHelpers.showImageState)(`defectImageV2_${idx + 1}`, 'success-state');
            ...
            if (maxIdx < v2Containers.length - 1) // Show the first empty defect image container to allow adding new defect images
        ...
        function showExtraImagesV2(extraImagesV2) {
            ...
            extraImagesV2.sort((a, b)=>a.name.localeCompare(b.name)).forEach((imageV2, idx)=>{
                ...
                (0, _sellItemHelpers.showImageState)(`extraImageV2_${idx + 1}`, 'success-state');
        ...
        function showImageV2AndHideSiblings(imageV2, imageName) {
            if (!imageV2?.url || !imageV2.name) return;
            const url = imageV2.versions?.small || imageV2.versions?.medium || imageV2.versions?.large || imageV2.url;
            document.getElementById(`${imageName}Preview`).style.backgroundImage = `url('${url}')`;
        ...
        for(let i = 0; i < options.length; i++){
            ...
            if (condition === optionValue) {
                ...
                if (condition === "Anv\xe4nd, tecken p\xe5 slitage" || condition === "Anv\xe4nd, tydligt slitage") defectInfoDiv.style.display = 'block';
                break;
            }
        }
...
function setUpEventListeners() {
    ...
    document.querySelectorAll('.w-file-remove-link').forEach((removeElement)=>{
        removeElement.addEventListener('click', ()=>{
            const container = removeElement.closest('.file-upload-container-v2') || removeElement.closest('.file-upload-container-edit-v2') || removeElement.closest('.file-upload-container-edit');
            if (!container) {
                console.log("No parent container found for removeElement: ", removeElement);
                return;
            }
            const id = container.querySelector('input').id;
            if (changedImages.includes(id)) changedImages = changedImages.filter((image)=>image !== id);
            else {
    ...
    [
        ...
        link.addEventListener('click', ()=>{
            ...
            const containersWithValue = visibleContainers.filter((container)=>{
                return container.querySelector('input').value?.length || container.querySelector('input').dataset.fileUrl?.length;
            }).length;
            ...
            if (numberOfContainers > 1 && containersWithValue !== allContainers.length - 1) container.style.display = 'none';
...
document.getElementById('cancelChatWithMai').addEventListener('click', ()=>{
    cancelFlow.style.display = 'none';
    darkOverlay.style.display = 'none';
    Intercom('showNewMessage', `Ang\xe5ende att avsluta f\xf6rs\xe4ljning f\xf6r plagg ${params.id}:\n\n`);
});
...
},{"./sellItemHelpers":"8xEoj","./autocomplete-brands":"aDjZV"}],"8xEoj":[function(require,module,exports,__globalThis) {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["vt3gJ","9IZ5h"], "9IZ5h", "parcelRequire81ca", {})
