...
async function getValuation(itemBrand, itemCategory) {
    ...
    try {
        const valuationRes = await firebase.app().functions("europe-west1").httpsCallable("partialMlValuation")({
            brand,
            category
        });
...
function showMenu(sessionUser) {
    let identifier = "";
    if (sessionUser.signInMethod.includes("google") || sessionUser.signInMethod.includes("password")) identifier = sessionUser.email;
    else if (sessionUser.signInMethod.includes("phone")) identifier = sessionUser.phoneNumber;
    if (identifier) {
    ...
    if (sessionUser?.referralData?.referralCode) menuInviteLink.style.display = "block";
