...
})({"6RTaC":[function(require,module,exports) {
...
async function getValuation(itemBrand, itemCategory) {
    ...
    try {
        const valuationRes = await (0, _general.callBackendApi)("/api/valuation?partial=true", {
            data: {
                brand,
                category
            }
        ...
        if (decline) {
            if (humanCheckExplanation?.match(/decline_blocked_brand/)) document.getElementById("itemValuationText").innerText = `Vi säljer generellt inte plagg från ${brand} på grund av för låg efterfrågan.`;
            else document.getElementById("itemValuationText").innerText = `Vi säljer generellt inte ${category.toLowerCase()} från ${brand} på grund av för låg efterfrågan.`;
            ...
            if (!fewBrand) {
                const startCopy = highPriceVarBrandCategory || brandCategoryAccuracy < 0.7 && brandAccuracy < 0.8 ? `Slutpriser för ${brand}-${category.toLowerCase()} kan variera mycket beroende på bl.a. modell och ålder. ` : "";
                const shareSoldInfo = brandShareSold >= 0.5 ? `${brand} är eftertraktat på andrahandsmarknaden, och mycket av det som läggs upp säljs.` : lowValueSegment ? `Efterfrågan på ${brand} är lite lägre än snittet, så det kan ta lite längre tid att sälja.` : "";
...
},{"./autocomplete-brands":"aDjZV","./sellItemHelpers":"8xEoj","./general":"lWrRo"}],"aDjZV":[function(require,module,exports) {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["6RTaC","7u85q"], "7u85q", "parcelRequire81ca")
