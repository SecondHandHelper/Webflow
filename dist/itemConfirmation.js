...
})({"97wxY":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "288e98bcd598cad6";
"use strict";
...
},{}],"4nhu0":[function(require,module,exports,__globalThis) {
var _sellItemHelpers = require("./sellItemHelpers");
function initializeFields(item1) {
    const { humanCheckNeeded, maxPriceEstimate: mlMaxPriceEstimate, newMinMaxLog } = item1.mlValuation || item1.estimatedValuation || {};
    if (item1.infoRequests?.price?.response === 'User proposal') {
    ...
    } else if (humanCheckNeeded || !mlMaxPriceEstimate && !item1.infoRequests?.price?.maxPrice || newMinMaxLog) {
        document.getElementById('nextStepTitle').style.display = 'block';
        document.getElementById('nextStepText').innerText = `Ditt ${item1.cleanedBrand || item1.brand}-plagg beh\xf6ver v\xe4rderas manuellt, d\xe5 AI-v\xe4rderingen har l\xe4gre tr\xe4ffs\xe4kerhet p\xe5 detta varum\xe4rke. Du kommer f\xe5 ett SMS n\xe4r vi v\xe4rderat plagget som du kan ta st\xe4llning till.`;
    }
    document.getElementById('itemTitle').innerText = (item1.cleanedBrand || item1.brand).trim() + "-" + item1.category.toLowerCase();
    document.getElementById('itemPrice').innerText = item1.maxPriceEstimate && !humanCheckNeeded && !newMinMaxLog ? `${item1.maxPriceEstimate || mlMaxPriceEstimate} SEK` : '';
    document.getElementById('itemPrice').style.display = 'block';
    document.getElementById('itemSubtitle').innerText = item1.model ? `${item1.model}, ${(0, _sellItemHelpers.colorName)(item1.color)}` : (0, _sellItemHelpers.colorName)(item1.color);
    document.getElementById('itemSize').innerText = item1.size;
    document.getElementById('itemMaterial').innerText = item1.material;
    document.getElementById('itemCondition').innerText = item1.condition = itemConditionText(item1);
    if (item1.originalPrice) {
        itemOriginalPrice.innerText = item1.originalPrice + ' SEK';
        originalPriceDiv.style.display = 'flex';
    }
    if (item1.userComment) {
        document.getElementById('itemComment').innerText = item1.userComment;
        document.getElementById('itemCommentDiv').style.display = 'block';
    }
    if (item1?.platformsToBePublishedOn?.length) initializePlatforms(item1);
    else document.getElementById('platformsSection').style.display = 'none';
}
function initializePlatforms(item1) {
    if (item1?.platformsToBePublishedOn?.length < 2) {
        document.getElementById('platformsSection').style.display = 'none';
        return;
    }
    const platformNode = document.getElementById('platformTradera');
    item1.platformsToBePublishedOn.forEach((platform)=>{
...
function itemConditionText(item1) {
    let conditionText = '';
    if (item1.condition === "Anv\xe4nd, tecken p\xe5 slitage" && (item1.defects.length || item1.defectDescription)) {
        let defects = [];
        if (item1.defectDescription) conditionText = item1.defectDescription;
        else if (item1.defects && item1.defects.length > 0) {
            defects = item1.defects.filter((defect)=>{
                return defect !== 'Annat';
            });
            conditionText = "Anm\xe4rkning: " + defects.join(', ');
        } else conditionText = item1.condition;
    } else conditionText = item1.condition;
...
const main = async ()=>{
    ...
    const item1 = params.id ? await getItem(params.id) : JSON.parse(localStorage.getItem('latestItemCreated'));
    if (!item1) {
        console.error("Invalid item id param or no recently created item");
        location.href = '/private';
    }
    initializeFields(item1);
...
function initializeSlider(item1) {
    ...
    for (const imageName of [
        ...
        if (imageName === 'frontImage' && images['enhancedFrontImage']) continue;
        console.log('imageName', imageName);
        const imageUrl = window.innerWidth <= 350 ? images[`${imageName}Small`] || images[imageName] : images[imageName];
        const newSlideHtml = `
    ...
    wrapper.addEventListener('touchend', ()=>{
        ...
        if (startX !== null && currentX !== null) {
            ...
            if (Math.abs(deltaX) > 50) {
                if (deltaX > 50) currentSlide = Math.max(currentSlide - 1, 0);
                else if (deltaX < -50) currentSlide = Math.min(currentSlide + 1, slides.length - 1);
            }
        }
...
const params = getParamsObject();
if (params.id) getItem(params.id).then(initializeSlider).catch((e)=>console.log('failed'));
else {
...
},{}]},["97wxY","4nhu0"], "4nhu0", "parcelRequire81ca")
