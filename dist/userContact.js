...
})({"YALB0":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "29b8cd08e4f4a30f";
"use strict";
...
},{}],"6mF3P":[function(require,module,exports,__globalThis) {
var _general = require("./general");
async function verifyUserCode() {
    const verificationCodeError = !verificationCode.value?.length || !/^[0-9]+$/.test(verificationCode.value) ? "Koden ska best\xe5 av siffror" : '';
...
const pageSetUp = async ()=>{
    const item = JSON.parse(sessionStorage.getItem('itemToBeCreatedAfterSignIn'))?.item || JSON.parse(localStorage.getItem('latestItemCreated'));
    if (item) {
        itemImage.src = item?.images?.enhancedFrontImageSmall || item?.images?.enhancedFrontImage || '';
        itemNotificationText.innerHTML = `Vi beh\xf6ver dina uppgifter f\xf6r att fullf\xf6lja f\xf6rs\xe4ljningen av ditt ${item.brand}-plagg. Anges bara en g\xe5ng.`;
    ...
    saveUserDetailsButton.addEventListener('click', async ()=>{
        const personalIdError = !personalId.value?.length || (0, _general.isValidSwedishSsn)(personalId.value) ? '' : 'Ogiltigt personnummer';
            ...
            try {
                const res = await callBackendApi('/api/users/saveAndVerify', {
                    data: {
                        data: {
                            phoneNumber: formatPhoneNumber(phoneNumber.value),
                            personalId: (0, _general.formatPersonalId)(personalId.value)
                        }
                    },
                    method: 'POST'
                });
...
},{}]},["YALB0","6mF3P"], "6mF3P", "parcelRequire81ca", {})
