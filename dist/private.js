...
function showInviteToast(items) {
    ...
    const daysSinceToastLastViewed = toastLastViewed ? Math.floor((nowDate.getTime() - toastLastViewed.getTime()) / 86400000) : null;
...
async function privateMain() {
    ...
    if (userId === "3OkW5av20HP8ScpUDS8ip9fBEZr1" && window.location.origin.includes("shh-test")) userId = "P9fYQomeZBW3jnYjnLFKurAJXYF3";
    //Yearly Summary
    yearlyDataExist(userId).then((result)=>{
        ...
        } else console.log("No yearly summary exist!");
    });
