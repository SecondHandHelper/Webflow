...
})({"gjEt0":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "628d4b663dd70c76";
"use strict";
...
},{}],"6uV5o":[function(require,module,exports,__globalThis) {
...
if (userIsSellingNewItem()) {
    ...
    notificationText.innerHTML = `F\xf6r att fullf\xf6lja f\xf6rs\xe4ljningen av ditt ${item.brand}-plagg beh\xf6ver du logga in eller skapa konto`;
...
function checkEmail() {
    ...
    firebase.auth().fetchSignInMethodsForEmail(email).then((signInMethods)=>{
        ...
        if (signInMethods.indexOf(firebase.auth.EmailAuthProvider.EMAIL_PASSWORD_SIGN_IN_METHOD) !== -1) {
            ...
            document.getElementById('prefilledEmail').innerText = `Ange l\xf6senord f\xf6r ${email}`;
        } else {
            // New account
            choosePasswordDiv.style.display = 'block';
            document.getElementById('createAccountEmail').innerText = `V\xe4lj l\xf6senord f\xf6r ${email}`;
        }
    }).catch((error)=>{
        errorHandler.report(error);
        console.log("Error message:", error.code, error.message);
...
},{}]},["gjEt0","6uV5o"], "6uV5o", "parcelRequire81ca")
