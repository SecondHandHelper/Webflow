...
})({"8lAQz":[function(require,module,exports,__globalThis) {
...
},{}],"bCEVa":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "minPriceMaxIncrease", ()=>minPriceMaxIncrease);
...
async function showDeclineValuation(item) {
    ...
    if (item.id) await callBackendApi(`/api/valuation/${item.id}/reject`, {
        data: {
            userDecline: false
        }
    });
...
const minTooHighWarning = (minPrice, adjustedMin)=>adjustmentRequiresReview(minPrice, adjustedMin) ? "Ovanligt h\xf6gt l\xe4gsta pris" : null;
...
const adjustmentValidations = (minPrice, maxPrice, adjustedMinInput, adjustedMaxInput)=>{
    ...
    if (adjustmentRequiresReview(minPrice, adjustedMin)) {
        ...
        if (adjustedMin > minPrice) noteText += `${noteText ? ' ' : ''}Ett h\xf6jt l\xe4gsta pris minskar sannolikheten att det blir s\xe5lt.`;
...
function validateInput() {
    ...
    const adjustmentMax1 = Number(adjustmentMaxInput.value);
    ...
    else if (adjustmentMin > adjustmentMax1) {
...
const initialPageSetup = (item)=>{
    ...
    document.getElementById('chatLink').onclick = ()=>Intercom('showNewMessage', item.mlValuation?.decline || item.estimatedValuation?.decline ? `ID: ${item.id}
...
const showValuation = async (item)=>{
    ...
    } else if (item.mlValuation) document.getElementById('valuationExplanation').innerText = getMlValuationExplanation(item);
...
const shortCondition = (condition)=>{
    return ({
        ...
        "Anv\xe4nd, men utan anm\xe4rkning": 'Nyskick',
        "Anv\xe4nd, tecken p\xe5 slitage": 'Fint skick',
...
const buildGroupingDescription = (item, grouping)=>{
    const attributeNames = grouping.map((attr)=>getAttributeDisplayName(attr));
    const attributeValues = grouping.map((attr)=>getAttributeDisplayValue(item, attr));
    // Filter out empty values
    const validAttributes = attributeNames.map((name, index)=>({
            name,
            value: attributeValues[index]
        })).filter((pair)=>pair.value && pair.value.trim() !== '');
    if (validAttributes.length === 0) return '';
    // Separate brand from other attributes
    const brandAttribute = validAttributes.find((attr)=>attr.name === "varum\xe4rke");
    const otherAttributes = validAttributes.filter((attr)=>attr.name !== "varum\xe4rke");
    // Build brand part (optional)
    let brandPart = '';
    if (brandAttribute) brandPart = `fr\xe5n ${brandAttribute.value} `;
    // Build attribute types part
    let attributeTypes = '';
    if (otherAttributes.length === 0) attributeTypes = 'kategori';
    else if (otherAttributes.length === 1) attributeTypes = otherAttributes[0].name;
    else if (otherAttributes.length === 2) attributeTypes = `${otherAttributes[0].name} och ${otherAttributes[1].name}`;
    else {
    ...
    return `Prisspannet \xe4r baserat p\xe5 liknande s\xe5lda ${brandPart} i samma ${attributeTypes}. Snittpriset f\xf6r dessa f\xf6rs\xe4ljningar ligger p\xe5 ${groupAvgPrice} kr.`;
};
const isUnknownModel = (item)=>{
    const includedBrandSegments = [
        '1A',
        '1B',
        '1C',
        '2A',
...
const getEstimatedPriceExplanation = (item)=>{
    if (!item.estimatedValuation) return '';
    let groupingText = '';
    if (item.estimatedValuation.grouping && item.estimatedValuation.grouping.length > 0) groupingText = buildGroupingDescription(item, item.estimatedValuation.grouping);
    if (isUnknownModel(item)) return `Vi hittar ingen exakt modell-match f\xf6r detta plagg, s\xe5 v\xe4rderingen \xe4r n\xe5got mer os\xe4ker. ${groupingText}`.trim();
    if (item.estimatedValuation.highUncertaintyFlag) return `${groupingText} Spannet vi presenterat har st\xf6rre os\xe4kerhet pga h\xf6g variation av slutpriser bland liknande s\xe5lda.`.trim();
    return groupingText;
};
...
const showAdjustValuation = async (item)=>{
    ...
    document.getElementById('maxPrice').addEventListener('input', ()=>{
        ...
        const adjustmentMinInput = document.getElementById('minPrice');
        const adjustmentMax1 = Number(adjustmentMaxInput.value);
        if (adjustmentMax1 !== maxPrice) document.getElementById('origMaxPrice').style.visibility = 'visible';
...
const maxIncrease = (price)=>{
    if (price <= 400) return price * 0.3;
    else if (price <= 800) return price * 0.25;
    return price * 0.2;
};
...
async function sellToMai(item) {
    ...
    /* Show spinner and call endpoint */ document.getElementById('sellToMaiBtnSpinner').style.display = 'block';
    ...
    /* When successful, show confirmation screen */ if (response?.data?.status === 'ok') {
        console.log('Item successfully sold to Mai:', item.id);
        document.getElementById('sellToMaiPopUpTitle').innerText = "S\xe5ld till Mai!";
        document.getElementById('sellToMaiPopUpText').innerText = "Om n\xe5gra minuter kommer du att f\xe5 ett SMS att plagget blivit s\xe5lt med l\xe4nk till QR-kod f\xf6r att skicka plagget till oss. Har du flera plagg du s\xe5lt till Mai s\xe5 kan du skicka i samma p\xe5se.";
    ...
    } else throw new Error('Could not save soldToMai');
}
function showSellToMaiOption(item) {
    ...
    /* Show sell to Mai option */ const sellNowPrice = Math.round(0.7 * minPrice);
    document.getElementById('sellToMaiDiv').style.display = 'flex';
    document.getElementById('openSellToMaiBtnText').innerText = `S\xe4lj nu f\xf6r ${sellNowPrice} kr`;
    /* Setup sell to Mai modal and buttons */ document.getElementById('sellNowPrice').innerText = `${sellNowPrice} kr`;
    document.getElementById('popupButtonText').innerText = `Ja, s\xe4lj till Mai f\xf6r ${sellNowPrice} kr`;
    ...
    if (image) document.getElementById('popUpImage').src = image;
}
const main = async ()=>{
...
},{}]},["8lAQz","bCEVa"], "bCEVa", "parcelRequire81ca", {})
